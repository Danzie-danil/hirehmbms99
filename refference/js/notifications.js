...
export function buildNotifItem(key, innerHtml, isRead = false, urgency = 'normal') {
    ...
    const readBtnHtml = isRead ? '' : `
        <button
            class="notif-read-btn absolute right-0 top-0 h-full px-3 flex flex-col items-center justify-center bg-indigo-500 text-white text-[9px] font-black uppercase tracking-widest opacity-0 group-hover/notif:opacity-100 transition-all duration-200 transform translate-x-full group-hover/notif:translate-x-0 z-10 hover:bg-indigo-600 rounded-r-xl min-w-[56px] gap-0.5"
    ...
    return `
    <div class="notif-wrapper group/notif relative overflow-hidden mb-2.5 rounded-xl ${readClass}"
         data-notif-key="${key}">
        ${readBtnHtml}
