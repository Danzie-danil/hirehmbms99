...
export function initSurveyRealtimeListener() {
    if (currentSurveyChannel) {
        try {
            supabase.removeChannel(currentSurveyChannel);
        } catch (e) {
            console.warn('[Surveys] Error cleaning up channel:', e);
        }
    }
    ...
    // Subscribe to realtime changes on sys_surveys
    try {
        currentSurveyChannel = supabase
            .channel('realtime_sys_surveys')
            .on(
                'postgres_changes',
                { event: '*', schema: 'public', table: 'sys_surveys' },
                (payload) => {
                    if (payload.eventType === 'INSERT' || payload.eventType === 'UPDATE') {
                        ...
                        if (survey && survey.status === 'active') {
                            ...
                            // If currently showing this closed survey, close it
                            if (activeSurveyInstance && activeSurveyInstance.id === survey.id) {
                                closeSurveyModal();
                            }
                        }
                    }
        ...
        console.warn('[Surveys] Realtime subscription error:', e);
    }
}
...
export async function checkAndShowActiveSurveys() {
    try {
        ...
        if (error || !surveys || !surveys.length) return;
        ...
        for (const survey of surveys) {
            ...
            const { data: existingResp } = await supabase
                .from('sys_survey_responses')
                .select('id')
                .eq('survey_id', survey.id)
                .eq('user_id', state.user?.id || (await supabase.auth.getUser())?.data?.user?.id)
                .maybeSingle();
    ...
    async function submitSurvey() {
        ...
        container.innerHTML = `
            <div class="py-12 flex flex-col items-center justify-center text-center space-y-4">
                ...
                <p class="text-xs font-bold text-gray-500">Submitting your response...</p>
            </div>
        `;
        ...
        try {
            ...
            const { data, error } = await supabase.rpc('submit_sys_survey_response', {
            ...
            // Fallback direct insert if RPC not present yet
            try {
                const authUser = (await supabase.auth.getUser())?.data?.user;
                await supabase.from('sys_survey_responses').insert({
                    survey_id: survey.id,
                    user_id: authUser?.id,
