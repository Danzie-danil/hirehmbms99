...
export function initReleaseNotesCheck() {
    setTimeout(() => {
        ...
        const justUpdated = sessionStorage.getItem('bms_just_updated') === 'true';
        ...
        // Case: First time install on this device/browser
        if (!lastSeen) {
            localStorage.setItem(INSTALLED_KEY, releaseData.version);
            localStorage.setItem(STORAGE_KEY, releaseData.version);
            return;
        ...
        // Case: Version upgrade detected or post-update verified reload
        if (justUpdated || lastSeen !== releaseData.version) {
