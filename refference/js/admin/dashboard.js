...
async function loadAdminData() {
    ...
    async function queryWithTimeout(queryPromise, timeoutMs = 12000) {
        try {
    ...
    try {
        const { data, error } = await queryWithTimeout(supabase.from('sys_settings').select('*'));
        ...
        console.warn('[Admin] Offline/timeout fallback for sys_settings:', e.message || e);
    }

    try {
        const { data, error } = await queryWithTimeout(supabase.from('profiles').select('*'));
        if (!error && Array.isArray(data)) {
            adminProfiles = data;
        }
    } catch (e) {
        console.warn('[Admin] Offline/timeout fallback for profiles:', e.message || e);
    }

    try {
        const { data, error } = await queryWithTimeout(supabase.from('sys_banners').select('*').order('created_at', { ascending: false }));
        if (!error && Array.isArray(data)) {
            adminBanners = data;
        }
    } catch (e) {
        console.warn('[Admin] Offline/timeout fallback for sys_banners:', e.message || e);
    }

    try {
        const { data, error } = await queryWithTimeout(supabase.from('branches').select('*'));
        if (!error && Array.isArray(data)) {
            adminBranches = data;
        }
    } catch (e) {
        console.warn('[Admin] Offline/timeout fallback for branches:', e.message || e);
    }

    try {
        const { data, error } = await queryWithTimeout(supabase.from('sys_email_drafts').select('*').order('updated_at', { ascending: false }));
        if (!error && Array.isArray(data)) {
            adminDrafts = data;
        }
    } catch (e) {
        console.warn('[Admin] Offline/timeout fallback for sys_email_drafts:', e.message || e);
    }

    try {
        const { data, error } = await queryWithTimeout(supabase.from('sys_pricing_plans').select('*'));
        if (!error && Array.isArray(data)) {
            adminPricingPlans = data;
        }
    } catch (e) {
        console.error('[Admin] Failed to load pricing plans from Supabase:', e);
    }

    // Persist latest state snapshot into IndexedDB safely
    try {
        await saveLocalSnapshot('sysadmin_global_state', 'sysadmin', 'all', {
            ...
            adminProfiles.forEach(profile => {
                ...
                <tr class="hover:bg-gray-50/50 dark:hover:bg-white/5 border-b border-gray-100 dark:border-gray-700/50 transition-colors">
                    <td class="px-5 sm:px-6 py-4">
                        <div class="font-bold text-gray-900 dark:text-white">${escapeHtml(profile.business_name || 'My Business')}</div>
                        <div class="text-xs text-gray-400 font-mono mb-1.5">${profile.id.substring(0, 8)}...</div>
                        <button onclick="openTenant360Modal('${profile.id}')" class="inline-flex items-center gap-1 text-[10px] font-bold text-indigo-600 dark:text-indigo-400 bg-indigo-50 dark:bg-indigo-900/20 hover:bg-indigo-100 dark:hover:bg-indigo-900/40 px-2.5 py-1 rounded-lg border border-indigo-100/50 dark:border-indigo-800/40 transition-all">
                            <i data-lucide="scan-face" class="w-3 h-3"></i> Tenant 360°
                        </button>
                    </td>
                    <td class="px-5 sm:px-6 py-4 text-sm text-gray-700 dark:text-gray-300">
                        <div class="font-semibold">${escapeHtml(profile.full_name || profile.business_name || 'Admin')}</div>
                        <div class="text-xs text-gray-400 font-mono">${escapeHtml(profile.email || profile.user_email || 'System Administrator')}</div>
                    </td>
                    <td class="px-5 sm:px-6 py-4">
                    ...
                    <td class="px-5 sm:px-6 py-4 text-xs text-gray-500 font-bold uppercase tracking-wider">
                    ...
                    <td class="px-5 sm:px-6 py-4 text-right">
                        <div class="flex items-center justify-end gap-1.5">
                            ${isPendingDeletion ? `
                                <button onclick="openSysadminDeletionManagerModal('${profile.id}')" class="p-2 text-amber-600 hover:bg-amber-50 dark:hover:bg-white/5 rounded-xl transition-all" title="Manage Deletion Pipeline">
                                    <i data-lucide="shield-alert" class="w-4 h-4"></i>
                                </button>
                            ` : ''}
                            <button onclick="openTenant360Modal('${profile.id}')" class="p-2 text-indigo-600 hover:bg-indigo-50 dark:hover:bg-white/5 rounded-xl transition-all" title="Inspect Tenant 360°">
                                <i data-lucide="scan-face" class="w-4 h-4"></i>
                            </button>
                            <button onclick="viewTenantBranches('${profile.id}')" class="p-2 text-emerald-600 hover:bg-emerald-50 dark:hover:bg-white/5 rounded-xl transition-all" title="View Branches (${myBranches.length})">
                                <i data-lucide="git-branch" class="w-4 h-4"></i>
                            </button>
                            <button onclick="editUserSubscription('${profile.id}')" class="p-2 text-amber-600 hover:bg-amber-50 dark:hover:bg-white/5 rounded-xl transition-all" title="Manage Subscription">
                ...
                <table class="w-full text-left border-collapse min-w-[640px]">
                    <thead>
                        <tr class="bg-gray-50/70 dark:bg-white/5 border-b border-gray-100 dark:border-gray-700/50 text-[11px] font-bold text-gray-400 dark:text-gray-500 uppercase tracking-wider">
                            <th class="px-5 sm:px-6 py-3.5 sm:py-4">Business</th>
                            <th class="px-5 sm:px-6 py-3.5 sm:py-4">Owner</th>
                            <th class="px-5 sm:px-6 py-3.5 sm:py-4">Plan</th>
                            <th class="px-5 sm:px-6 py-3.5 sm:py-4">Subscription Expiry</th>
                            <th class="px-5 sm:px-6 py-3.5 sm:py-4">Status</th>
                            <th class="px-5 sm:px-6 py-3.5 sm:py-4 text-right">Actions</th>
                        </tr>
                    </thead>
                    <tbody id="adminUserTableBody" class="divide-y divide-gray-100 dark:divide-gray-700/40 text-xs sm:text-sm">
                        ${rowsHtml}
                    </tbody>
                </table>
            </div>
...
window.editUserSubscription = async function(profileId) {
    ...
    let optionsHtml = `<option value="free_trial" ${(!profile.plan || profile.plan === 'free_trial') ? 'selected' : ''}>Free Trial</option>`;
    
    if (adminPricingPlans && adminPricingPlans.length > 0) {
        adminPricingPlans.forEach(plan => {
            const planKey = plan.plan_name.toLowerCase();
            const isSelected = profile.plan && profile.plan.toLowerCase() === planKey;
            const priceFmt = `TSh ${Number(plan.price).toLocaleString()} / mo`;
            const limitFmt = plan.max_branches >= 9999 ? 'Unlimited Branches' : `Up to ${plan.max_branches} Branches`;
            optionsHtml += `<option value="${planKey}" ${isSelected ? 'selected' : ''}>${plan.plan_name} (${priceFmt} - ${limitFmt})</option>`;
        });
    } else {
        optionsHtml += `
    ...
    <div class="p-5 sm:p-7 space-y-5">
        <div class="border-b border-gray-100 dark:border-gray-800 pb-4">
            ...
            <p class="text-[10px] font-bold text-gray-400 dark:text-gray-500 uppercase tracking-widest mt-0.5">${profile.business_name || 'Business Settings'}</p>
        </div>

        <div class="space-y-4">
            <div>
                <label for="adminPlanSelect" class="block text-xs font-bold text-gray-700 dark:text-gray-300 mb-1.5">Pricing Plan</label>
                <select id="adminPlanSelect" class="form-input text-xs sm:text-sm">
                    ${optionsHtml}
                </select>
            </div>

            <div>
                <label for="adminTrialEndsInput" class="block text-xs font-bold text-gray-700 dark:text-gray-300 mb-1.5">Subscription Expiration Date</label>
                ${window.renderPremiumDatePicker ? window.renderPremiumDatePicker({
                    id: 'adminTrialEndsInput',
                    selectedValue: formattedDate,
                    placeholder: 'Select Expiration Date',
                    classes: 'w-full'
                }) : `<input type="date" id="adminTrialEndsInput" value="${formattedDate}" class="form-input text-xs sm:text-sm">`}
            </div>
        </div>

        <div class="flex gap-3 pt-4 border-t border-gray-100 dark:border-gray-800">
            <button onclick="closeModal()" class="flex-1 py-2.5 px-4 text-xs sm:text-sm font-bold text-gray-700 dark:text-gray-300 bg-gray-100 dark:bg-gray-800 hover:bg-gray-200 dark:hover:bg-gray-700 rounded-xl transition-all">Cancel</button>
            <button onclick="saveUserSubscription('${profileId}')" class="flex-1 py-2.5 px-4 text-xs sm:text-sm font-bold text-white bg-indigo-600 hover:bg-indigo-700 active:scale-95 rounded-xl shadow-md shadow-indigo-600/20 transition-all">Save Changes</button>
        </div>
    </div>
    `;
...
window.saveUserSubscription = async function(profileId) {
    const newPlan = document.getElementById('adminPlanSelect').value;
    ...
    const payload = {
        plan: newPlan,
        trial_ends_at: newExpiry ? new Date(newExpiry).toISOString() : null,
    ...
    try {
        ...
        closeModal();
        showToast('Local sync complete (DB offline/unauthorized)', 'warning');
        renderUserMaintenance();
    }
};
...
function renderSysadminProfile() {
    ...
    mainContent.innerHTML = `
    <div class="space-y-8 slide-in w-full max-w-2xl mx-auto">
        <div>
            <h1 class="text-3xl font-black text-gray-900 dark:text-white mb-1">Admin Profile</h1>
            <p class="text-sm text-gray-500 dark:text-gray-400">System administrator account details</p>
        </div>
...
window.sysadminPurgeTenantPermanently = async function(tenantId, businessName) {
    ...
    try {
        ...
        if (!rpcSuccess) {
            ...
            await supabase.from('suppliers').delete().eq('owner_id', tenantId);
