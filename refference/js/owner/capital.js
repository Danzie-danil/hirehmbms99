...
function renderCapitalFilterPillsHTML(accounts = [], branches = []) {
    ...
    return `
        <button onclick="window.setCapitalListFilter('all')" class="px-3.5 py-1.5 rounded-xl font-bold text-xs transition-all whitespace-nowrap cursor-pointer ${activeFilter === 'all' ? 'bg-indigo-600 text-white shadow-xs' : 'bg-gray-100 dark:bg-gray-700 text-gray-600 dark:text-gray-300 hover:bg-gray-200'}">
            All Accounts (${accounts.length})
        </button>
        <button onclick="window.setCapitalListFilter('bank')" class="px-3.5 py-1.5 rounded-xl font-bold text-xs transition-all whitespace-nowrap cursor-pointer ${activeFilter === 'bank' ? 'bg-blue-600 text-white shadow-xs' : 'bg-gray-100 dark:bg-gray-700 text-gray-600 dark:text-gray-300 hover:bg-gray-200'}">
            🏦 Bank Accounts (${accounts.filter(a => a.account_type === 'bank').length})
        </button>
        <button onclick="window.setCapitalListFilter('mobile_money')" class="px-3.5 py-1.5 rounded-xl font-bold text-xs transition-all whitespace-nowrap cursor-pointer ${activeFilter === 'mobile_money' ? 'bg-emerald-600 text-white shadow-xs' : 'bg-gray-100 dark:bg-gray-700 text-gray-600 dark:text-gray-300 hover:bg-gray-200'}">
            📱 Mobile Money Tills (${accounts.filter(a => a.account_type === 'mobile_money').length})
        </button>
        <button onclick="window.setCapitalListFilter('cash')" class="px-3.5 py-1.5 rounded-xl font-bold text-xs transition-all whitespace-nowrap cursor-pointer ${activeFilter === 'cash' ? 'bg-amber-600 text-white shadow-xs' : 'bg-gray-100 dark:bg-gray-700 text-gray-600 dark:text-gray-300 hover:bg-gray-200'}">
            💵 Physical Cash Drawers (${accounts.filter(a => ['cash', 'petty_cash'].includes(a.account_type)).length})
        </button>
        ${branches.map(b => `
            <button onclick="window.setCapitalListFilter('branch_${b.id}')" class="px-3.5 py-1.5 rounded-xl font-bold text-xs transition-all whitespace-nowrap cursor-pointer ${activeFilter === ('branch_' + b.id) ? 'bg-purple-600 text-white shadow-xs' : 'bg-gray-100 dark:bg-gray-700 text-gray-600 dark:text-gray-300 hover:bg-gray-200'}">
                📍 ${b.name} (${accounts.filter(a => a.branch_id === b.id).length})
            </button>
        `).join('')}
    `;
}
...
function renderCapitalAccountsGridHTML(accounts = [], branchMap = new Map()) {
    ...
    return filteredAccounts.map(a => `
        <div class="border border-gray-200/80 dark:border-gray-700/80 rounded-2xl p-4 bg-slate-50/50 dark:bg-gray-900/40 hover:border-indigo-300 transition-all flex flex-col justify-between space-y-3">
            <div>
                <div class="flex items-center justify-between mb-2">
                    <span class="text-xs font-bold uppercase px-2.5 py-0.5 rounded-lg border ${a.account_type === 'bank' ? 'bg-blue-50 text-blue-700 border-blue-200 dark:bg-blue-950/40 dark:text-blue-300 dark:border-blue-900' : a.account_type === 'mobile_money' ? 'bg-emerald-50 text-emerald-700 border-emerald-200 dark:bg-emerald-950/40 dark:text-emerald-300 dark:border-emerald-900' : 'bg-amber-50 text-amber-700 border-amber-200 dark:bg-amber-950/40 dark:text-amber-300 dark:border-amber-900'}">
                ...
                <h4 class="font-bold text-gray-900 dark:text-white text-base leading-tight">${a.account_name}</h4>
                ${a.account_number ? `<p class="text-xs text-gray-400 font-mono mt-0.5">${a.bank_name ? a.bank_name + ' • ' : ''}${a.account_number}</p>` : ''}
            </div>
            <div class="flex items-center justify-between pt-2 border-t border-gray-200/60 dark:border-gray-750">
                <div>
                    ...
                    <p class="text-lg font-black text-gray-900 dark:text-white">${window.fmt.currency(a.balance || 0)}</p>
                </div>
                <div class="flex items-center gap-1">
                    <button onclick="window.renderAddCapitalAccountView('${a.id}')" title="Edit Account Details" class="p-2 text-gray-400 hover:text-indigo-600 dark:hover:text-indigo-400 rounded-xl hover:bg-indigo-50 dark:hover:bg-indigo-950/30 transition-colors cursor-pointer">
                        <i data-lucide="edit-2" class="w-4 h-4"></i>
                    </button>
                    <button onclick="window.deleteCapitalAccount('${a.id}')" title="Delete Account" class="p-2 text-gray-400 hover:text-red-600 dark:hover:text-red-400 rounded-xl hover:bg-red-50 dark:hover:bg-red-950/30 transition-colors cursor-pointer">
...
export async function renderOwnerCapitalModule() {
    ...
    try {
        const [accounts, transactions, branches] = await Promise.all([
            ...
            <div class="space-y-6">
                ...
                <div class="flex flex-col sm:flex-row sm:items-center sm:justify-between gap-4 bg-white dark:bg-gray-800 p-5 sm:p-6 rounded-2xl border border-gray-200/80 dark:border-gray-700/80 shadow-xs">
                    <div class="space-y-1">
                        ...
                        <h2 class="text-xl sm:text-2xl font-black text-gray-900 dark:text-white tracking-tight">Business Capital & Liquidity</h2>
                        <p class="text-xs text-gray-500 dark:text-gray-400">Track liquid cash, bank balances, mobile money tills & owner capital deposits</p>
                    </div>
                    <div class="flex flex-col items-stretch sm:items-end gap-2 shrink-0 w-full sm:w-auto">
                        <button onclick="window.renderAddCapitalAccountView()" class="w-full sm:w-auto px-4 py-2 bg-[#475B6E] hover:bg-[#394a5a] text-white font-bold text-xs rounded-xl transition-all flex items-center justify-center gap-1.5 shadow-xs cursor-pointer">
                            <i data-lucide="plus" class="w-4 h-4"></i> Add Account (Single / Batch)
                        </button>
                        <button onclick="window.renderAddCapitalTransactionView()" class="w-full sm:w-auto px-4 py-2 bg-emerald-600 hover:bg-emerald-700 text-white font-bold text-xs rounded-xl transition-all flex items-center justify-center gap-1.5 shadow-xs cursor-pointer">
                            <i data-lucide="arrow-up-down" class="w-4 h-4"></i> Deposit / Withdrawal
                        </button>
                    </div>
                </div>
                ...
                <div class="grid grid-cols-2 lg:grid-cols-4 gap-3 sm:gap-4">
                    <div class="relative bg-white dark:bg-gray-800/90 border border-gray-200/80 dark:border-gray-700/80 rounded-2xl p-4 shadow-xs flex flex-col justify-between h-full stat-card min-w-0">
                        ...
                        <div class="flex items-center justify-between text-gray-500 dark:text-gray-400 mb-2">
                            <span class="text-xs font-bold uppercase tracking-wider">Total Available Capital</span>
                        ...
                        <p class="text-dynamic-lg font-black text-gray-900 dark:text-white truncate leading-none my-auto py-1" title="${window.fmt.currency(totalLiquidCapital)}">${window.fmt.number(totalLiquidCapital)}</p>
                        <p class="text-[11px] text-gray-400 mt-1">Across all accounts & tills</p>
                    </div>
                    ...
                    <div class="relative bg-white dark:bg-gray-800/90 border border-gray-200/80 dark:border-gray-700/80 rounded-2xl p-4 shadow-xs flex flex-col justify-between h-full stat-card min-w-0">
                        ...
                        <div class="flex items-center justify-between text-gray-500 dark:text-gray-400 mb-2">
                            <span class="text-xs font-bold uppercase tracking-wider">Bank Balance</span>
                        ...
                        <p class="text-dynamic-lg font-black text-blue-600 dark:text-blue-400 truncate leading-none my-auto py-1" title="${window.fmt.currency(totalBank)}">${window.fmt.number(totalBank)}</p>
                        <p class="text-[11px] text-gray-400 mt-1">Commercial bank accounts</p>
                    </div>
                    ...
                    <div class="relative bg-white dark:bg-gray-800/90 border border-gray-200/80 dark:border-gray-700/80 rounded-2xl p-4 shadow-xs flex flex-col justify-between h-full stat-card min-w-0">
                        ...
                        <div class="flex items-center justify-between text-gray-500 dark:text-gray-400 mb-2">
                            <span class="text-xs font-bold uppercase tracking-wider">Mobile Money</span>
                        ...
                        <p class="text-dynamic-lg font-black text-emerald-600 dark:text-emerald-400 truncate leading-none my-auto py-1" title="${window.fmt.currency(totalMobileMoney)}">${window.fmt.number(totalMobileMoney)}</p>
                        <p class="text-[11px] text-gray-400 mt-1">M-Pesa / Tigo Pesa / Airtel</p>
                    </div>
                    ...
                    <div class="relative bg-white dark:bg-gray-800/90 border border-gray-200/80 dark:border-gray-700/80 rounded-2xl p-4 shadow-xs flex flex-col justify-between h-full stat-card min-w-0">
                        ...
                        <div class="flex items-center justify-between text-gray-500 dark:text-gray-400 mb-2">
                            <span class="text-xs font-bold uppercase tracking-wider">Cash Balance</span>
                        ...
                        <p class="text-dynamic-lg font-black text-amber-600 dark:text-amber-400 truncate leading-none my-auto py-1" title="${window.fmt.currency(totalCash)}">${window.fmt.number(totalCash)}</p>
                        <p class="text-[11px] text-gray-400 mt-1">Shop tills & petty cash</p>
                    </div>
                </div>
                ...
                <div class="bg-white dark:bg-gray-800/90 border border-gray-200/80 dark:border-gray-700/80 rounded-3xl p-5 sm:p-6 shadow-xs space-y-4">
                    <div class="flex flex-col sm:flex-row sm:items-center justify-between gap-4">
                        <div>
                            <h3 class="text-lg font-bold text-gray-900 dark:text-white">Capital Accounts Custom Lists</h3>
                        ...
                        <div class="relative w-full sm:w-64">
                            ...
                            <input type="text" value="${state._capitalListSearch || ''}" oninput="window.setCapitalSearch(this.value)" placeholder="Search account name or number..." class="form-input w-full pl-9 py-1.5 text-xs font-bold rounded-xl">
                        </div>
                    </div>
                ...
                <div class="bg-white dark:bg-gray-800/90 border border-gray-200/80 dark:border-gray-700/80 rounded-3xl p-5 sm:p-6 shadow-xs space-y-4">
                    <div class="flex items-center justify-between">
                        <div>
                            <h3 class="text-lg font-bold text-gray-900 dark:text-white">Capital Deposits & Withdrawals Log</h3>
                        ...
                        <table class="w-full text-left text-xs">
                            <thead class="bg-gray-50 dark:bg-gray-900/50 text-gray-500 uppercase text-[10px] font-bold tracking-wider border-b border-gray-200 dark:border-gray-700">
                                <tr>
                                    ...
                                    <th class="p-3">Capital Account</th>
                            ...
                            <tbody class="divide-y divide-gray-100 dark:divide-gray-800">
                                ${transactions.length > 0 ? transactions.map(t => {
                                    ...
                                    <tr class="hover:bg-gray-50/50 dark:hover:bg-gray-800/50 transition-colors">
                                        <td class="p-3 font-medium text-gray-900 dark:text-white">${t.transaction_date}</td>
                                        <td class="p-3">
                                            <span class="px-2.5 py-0.5 rounded-lg text-[10px] font-black uppercase border ${isDep ? 'bg-emerald-50 text-emerald-700 border-emerald-200 dark:bg-emerald-950/40 dark:text-emerald-300' : 'bg-red-50 text-red-700 border-red-200 dark:bg-red-950/40 dark:text-red-300'}">
                                                ${isDep ? '📥 Deposit' : '📤 Withdrawal'}
                                            </span>
                                        </td>
                                        <td class="p-3 font-bold text-gray-800 dark:text-gray-200">${t.account_id ? (accountMap.get(t.account_id) || 'Capital Account') : 'General Capital'}</td>
                                        <td class="p-3 font-bold ${isDep ? 'text-emerald-600' : 'text-red-600'}">${window.fmt.currency(t.amount || 0)}</td>
                                        <td class="p-3 text-gray-500 dark:text-gray-400">${t.notes || t.reference_no || '-'}</td>
                                    </tr>
                                `;}).join('') : `
                                    <tr>
...
function setupRealtimeCapitalSubscription(ownerId) {
    ...
    try {
        realtimeChannel = supabase
            .channel('public:capital_realtime')
            .on('postgres_changes', { event: '*', schema: 'public', table: 'capital_accounts', filter: `owner_id=eq.${ownerId}` }, () => {
                if (document.getElementById('mainContent')?.querySelector('[data-view="capital"]') || window.location.hash.includes('capital')) {
                    window.renderOwnerCapitalModule();
                }
            })
            .on('postgres_changes', { event: '*', schema: 'public', table: 'capital_transactions', filter: `owner_id=eq.${ownerId}` }, () => {
                if (document.getElementById('mainContent')?.querySelector('[data-view="capital"]') || window.location.hash.includes('capital')) {
                    window.renderOwnerCapitalModule();
                }
            })
        ...
        console.warn('[OwnerCapital] Realtime subscription error:', e);
    }
}
