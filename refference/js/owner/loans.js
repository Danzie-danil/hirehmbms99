            ...
            <div class="space-y-6">
                ...
                <div class="grid grid-cols-2 lg:grid-cols-4 gap-3 sm:gap-4">
                    <div class="relative bg-white dark:bg-gray-800/90 border border-gray-200/80 dark:border-gray-700/80 rounded-2xl p-4 shadow-xs flex flex-col justify-between h-full stat-card min-w-0">
                        ...
                        <div class="flex items-center justify-between text-gray-500 dark:text-gray-400 mb-2">
                            <span class="text-xs font-bold uppercase tracking-wider">Total Principal Borrowed</span>
                            <div class="w-8 h-8 rounded-xl bg-slate-100 dark:bg-gray-700 text-gray-700 dark:text-gray-300 flex items-center justify-center font-bold">
                                <i data-lucide="landmark" class="w-4 h-4"></i>
                            </div>
                        </div>
                        <h3 class="text-xl sm:text-2xl font-black text-gray-900 dark:text-white truncate my-1" title="${window.fmt.currency(totalPrincipal)}">${window.fmt.number(totalPrincipal)}</h3>
                        <p class="text-[11px] text-gray-400 mt-1">Cumulative original debt</p>
                    </div>
                    ...
                    <div class="relative bg-white dark:bg-gray-800/90 border border-gray-200/80 dark:border-gray-700/80 rounded-2xl p-4 shadow-xs flex flex-col justify-between h-full stat-card min-w-0">
                        ...
                        <div class="flex items-center justify-between text-gray-500 dark:text-gray-400 mb-2">
                            <span class="text-xs font-bold uppercase tracking-wider">Outstanding Debt Balance</span>
                        ...
                        <h3 class="text-xl sm:text-2xl font-black text-rose-600 dark:text-rose-400 truncate my-1" title="${window.fmt.currency(totalRemaining)}">${window.fmt.number(totalRemaining)}</h3>
                        <p class="text-[11px] text-gray-400 mt-1">Active liabilities remaining</p>
                    </div>
                    ...
                    <div class="relative bg-white dark:bg-gray-800/90 border border-gray-200/80 dark:border-gray-700/80 rounded-2xl p-4 shadow-xs flex flex-col justify-between h-full stat-card min-w-0">
                        ...
                        <div class="flex items-center justify-between text-gray-500 dark:text-gray-400 mb-2">
                            <span class="text-xs font-bold uppercase tracking-wider">Total Repaid to Date</span>
                        ...
                        <h3 class="text-xl sm:text-2xl font-black text-emerald-600 dark:text-emerald-400 truncate my-1" title="${window.fmt.currency(totalRepaid)}">${window.fmt.number(totalRepaid)}</h3>
                        <p class="text-[11px] text-gray-400 mt-1">Cleared debt & installments</p>
                    </div>

                    <div class="relative bg-white dark:bg-gray-800/90 border border-gray-200/80 dark:border-gray-700/80 rounded-2xl p-4 shadow-xs flex flex-col justify-between h-full stat-card min-w-0">
                        <div class="flex items-center justify-between text-gray-500 dark:text-gray-400 mb-2">
                            <span class="text-xs font-bold uppercase tracking-wider">Active Credit Lines</span>
                        ...
                        <h3 class="text-xl sm:text-2xl font-black text-gray-900 dark:text-white truncate my-1">${activeLoansCount} Active</h3>
                        <p class="text-[11px] text-gray-400 mt-1">Open bank & supplier facilities</p>
                    </div>
                </div>
                ...
                <div class="bg-white dark:bg-gray-800/90 border border-gray-200/80 dark:border-gray-700/80 rounded-3xl p-5 sm:p-6 shadow-xs space-y-4">
                    <div class="flex flex-col sm:flex-row sm:items-center justify-between gap-4">
                        <div>
                            <h3 class="text-lg font-bold text-gray-900 dark:text-white">Business Liabilities Custom Lists</h3>
                ...
                <div class="bg-white dark:bg-gray-800/90 border border-gray-200/80 dark:border-gray-700/80 rounded-3xl p-5 sm:p-6 shadow-xs space-y-4">
                    <div class="flex items-center justify-between">
                        <div>
                            <h3 class="text-lg font-bold text-gray-900 dark:text-white">Loan Repayment History</h3>
...
function renderLoansGridHTML(filteredLoans) {
    ...
    return filteredLoans.map(l => `
        <div class="border border-gray-200/80 dark:border-gray-700/80 rounded-2xl p-4 bg-slate-50/50 dark:bg-gray-900/40 hover:border-rose-300 transition-all flex flex-col justify-between space-y-3">
            <div>
                <div class="flex items-center justify-between mb-2">
                    <span class="text-xs font-bold uppercase px-2.5 py-0.5 rounded-lg border ${l.status === 'active' ? 'bg-rose-50 text-rose-700 border-rose-200 dark:bg-rose-950/40 dark:text-rose-300' : 'bg-emerald-50 text-emerald-700 border-emerald-200 dark:bg-emerald-950/40 dark:text-emerald-300'}">
                        ${l.status.replace('_', ' ')}
                    </span>
                    <span class="text-[11px] text-gray-500 font-bold">${l.loan_type.replace('_', ' ').toUpperCase()}</span>
                </div>
                <h4 class="font-bold text-gray-900 dark:text-white text-base leading-tight">${l.lender_name}</h4>
                ${l.account_number ? `<p class="text-xs text-gray-400 font-mono mt-0.5">Acc: ${l.account_number}</p>` : ''}
            </div>
...
function setupRealtimeLoansSubscription(ownerId) {
    ...
    try {
        realtimeChannel = supabase
            .channel('public:loans_realtime')
            .on('postgres_changes', { event: '*', schema: 'public', table: 'business_loans', filter: `owner_id=eq.${ownerId}` }, () => {
                if (document.getElementById('mainContent')?.querySelector('[data-view="loans"]') || window.location.hash.includes('loans')) {
                    window.renderOwnerLoansModule();
                }
            })
            .on('postgres_changes', { event: '*', schema: 'public', table: 'loan_repayments', filter: `owner_id=eq.${ownerId}` }, () => {
                if (document.getElementById('mainContent')?.querySelector('[data-view="loans"]') || window.location.hash.includes('loans')) {
                    window.renderOwnerLoansModule();
                }
            })
        ...
        console.warn('[OwnerLoans] Realtime subscription error:', e);
    }
}
