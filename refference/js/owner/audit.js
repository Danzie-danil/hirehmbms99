...
export async function renderAuditModule() {
    ...
    try {
        ...
        const [salesRes, expRes, taskRes, reqRes] = await Promise.all([
            supabaseClient.from('sales').select('id, amount, created_at, branch_id, payment').in('branch_id', targetBIds).gte('created_at', since).order('created_at', { ascending: false }).limit(80),
            supabaseClient.from('expenses').select('id, amount, category, description, created_at, branch_id').in('branch_id', targetBIds).gte('created_at', since).order('created_at', { ascending: false }).limit(80),
            supabaseClient.from('tasks').select('id, title, status, created_at, branch_id').in('branch_id', targetBIds).gte('created_at', since).order('created_at', { ascending: false }).limit(80),
            supabaseClient.from('requests').select('id, subject, type, status, created_at').eq('owner_id', ownerId).gte('created_at', since).order('created_at', { ascending: false }).limit(50)
        ]);

        const branchMap = Object.fromEntries(branches.map(b => [b.id, b.name]));

        let events = [
            ...(salesRes.data || []).map(s => ({ id: s.id, time: s.created_at, type: 'sale', branchId: s.branch_id, label: `Sale recorded — ${fmt.currency(s.amount)} (${s.payment || s.payment_method || 'cash'})`, icon: 'shopping-cart', color: 'bg-emerald-100 text-emerald-600' })),
            ...(expRes.data || []).map(e => ({ id: e.id, time: e.created_at, type: 'expense', branchId: e.branch_id, label: `Expense: ${e.description || e.category} — ${fmt.currency(e.amount)}`, icon: 'credit-card', color: 'bg-red-100 text-red-600' })),
            ...(taskRes.data || []).map(t => ({ id: t.id, time: t.created_at, type: 'task', branchId: t.branch_id, label: `Task ${t.status}: "${t.title}"`, icon: 'check-square', color: 'bg-blue-100 text-blue-600' })),
            ...(reqRes.data || []).map(r => ({ id: r.id, time: r.created_at, type: 'request', branchId: null, label: `Request: ${r.subject} (${r.status})`, icon: 'shield-check', color: 'bg-purple-100 text-purple-600' }))
        ];
