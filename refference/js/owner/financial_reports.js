...
window.triggerReportDatePicker = triggerReportDatePicker;

let _reportCategory = 'financial_pl';
            ...
            <div class="modal-main-content p-3.5 sm:p-5 space-y-3.5 scroller-custom" id="ownerReportsScrollBody">
                ...
                <div class="bg-white dark:bg-gray-800 p-4 sm:p-5 rounded-2xl border border-gray-200/80 dark:border-gray-700/60 shadow-xs space-y-3.5">
                    <div class="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-12 gap-3 sm:gap-3.5">
                        ...
                        <!-- Custom Date Range Selector (Custom Calendar Popover Component) -->
                        ...
                        <tbody class="divide-y divide-gray-100 dark:divide-gray-700/60">
                            ...
                            data.sales.slice(0, 20).map(s => `
                                <tr class="hover:bg-gray-50/70 dark:hover:bg-gray-700/40 transition-colors">
                                    ...
                                    <td class="px-4 py-3 text-gray-700 dark:text-gray-300 font-medium">${data.branchMap.get(s.branch_id)?.name || 'Store'}</td>
...
export async function openDownloadReportsModal(customBranchId = null) {
    ...
    const branches = await dbBranches.fetchAll(window.state?.ownerId);
    const branchName = targetBranch === 'all' ? 'All Branches (Consolidated)' : (branches.find(b => b.id === targetBranch)?.name || 'Selected Branch');
            ...
            <div class="bg-white dark:bg-gray-800 px-4 py-2 border-b border-gray-200/60 dark:border-gray-700/60 flex items-center justify-between gap-2 text-xs flex-none">
                <div class="flex items-center gap-2 min-w-0 overflow-hidden">
                    ...
                    <span class="inline-flex items-center gap-1 text-gray-500 dark:text-gray-400 text-[11px] font-medium whitespace-nowrap truncate">
                        ...
                        <span>${formatDate(_reportFrom)} – ${formatDate(_reportTo)}</span>
                    </span>
                </div>
...
window.applyTimeframePreset = function(preset) {
    ...
    if (preset === 'today') {
        ...
        _reportFrom = monday.toISOString().slice(0, 10);
        _reportTo = now.toISOString().slice(0, 10);
    } else if (preset === 'this_month') {
        _reportFrom = new Date(now.getFullYear(), now.getMonth(), 1).toISOString().slice(0, 10);
        _reportTo = now.toISOString().slice(0, 10);
    } else if (preset === 'this_quarter') {
        const quarterMonth = Math.floor(now.getMonth() / 3) * 3;
        _reportFrom = new Date(now.getFullYear(), quarterMonth, 1).toISOString().slice(0, 10);
        _reportTo = now.toISOString().slice(0, 10);
    } else if (preset === 'this_year') {
        _reportFrom = new Date(now.getFullYear(), 0, 1).toISOString().slice(0, 10);
        _reportTo = now.toISOString().slice(0, 10);
    }
