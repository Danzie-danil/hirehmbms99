...
export function setupDashboard() {
    ...
    if (state.role && typeof window.applyDashboardRole === 'function') {
...
export function setLoginRole(role) {
    ...
    if (role === 'owner') {
        ...
        if (slider) slider.style.transform = 'translateX(100%) translateX(4px)';
...
function _tryOptimisticRestore() {
    try {
        ...
        if (!cachedRaw && activeUserId) {
            for (const r of ['owner', 'branch', 'sysadmin']) {
...
async function _executePendingLogin({ role, email, password }) {
    ...
    try {
        ...
        // Hydrate Supabase JS client session non-blockingly in background
        log('⚡ Hydrating Supabase client session...');
        supabase.auth.setSession({
            access_token: tokenData.access_token,
            refresh_token: tokenData.refresh_token
        }).catch(err => console.warn('[Auth] setSession background notice:', err));
        ...
        // Server-side RPC validation check with explicit bearer token header
        try {
            ...
            if (rpcErr) {
                ...
                dbAuth.signOut().catch(() => {});
                _fail(rpcErr.message || 'Access denied by authentication policy.');
                if (role === 'owner') {
        ...
        if (role === 'owner') {
            ...
            Promise.allSettled([
                ...
                if (!profile) {
                    dbProfile.upsert(user.id, {
                        full_name: user.user_metadata?.first_name || 'Admin',
                        currency: 'TZS',
                        email: user.email
                    }).then(p => { if (p) state.profile = p; }).catch(() => {});
                    state.profile = { currency: 'TZS', email: user.email };
                } else {
                    state.profile = profile;
            ...
            const initialBranchView = localStorage.getItem('lastBranchView') || 'dashboard';
            if (typeof window.switchView === 'function') {
                window.switchView(initialBranchView, null, true);
            }
...
export async function initAuth() {
    if (window.location.pathname === '/app/systemadmin' || window.location.pathname === '/app/systemadmin/') {
        unlockAdminPortalUI();
    }
    ...
    // 2. Fetch authenticated session with a fail-safe timeout (4.0s)
    ...
    try {
        ...
        console.warn('[Auth] dbAuth.getSession background timeout or error:', e);
        _authInitDone = true; // Release sign-in wait before showing login/error UI
        if (hadOptimisticRestore || _tryRestoreOfflineSession()) {
            console.log('[Auth] Kept persistent cached session during connection wait/sleep.');
            return;
        }
    ...
    // 3. No session found — skip refreshSession() to avoid holding the Supabase auth lock.
    // refreshSession() acquires an internal gotrue-js lock. If it runs while the user
    // clicks Sign In, signInWithPassword() deadlocks waiting for that lock to release,
    // causing the "Signing in..." button to appear frozen indefinitely.

    // 4. No session on backend -> check if local cached session exists before showing login screen
    if (!session || !session.user) {
        if (hadOptimisticRestore || _tryRestoreOfflineSession()) {
            console.log('[Auth] Persistent cached session active.');
            return;
        }

        // Reset memory state so no background queries or polling loops execute when unauthenticated
        state.profile = null;
        state.branchProfile = null;
        state.ownerId = null;
        state.branchId = null;
        ...
        setupAdminPasscodeListener();
        _authInitDone = true; // Lock released — Sign In button can now fire signIn safely
        hideLoader();
        hideInitialLoader();
        return;
    }
    ...
    try {
        ...
        const branchCandidate = managerBranchRes.status === 'fulfilled' ? managerBranchRes.value : null;
        const branch = (branchCandidate && branchCandidate.owner_id !== session.user.id) ? branchCandidate : null;
        ...
        if (branch) {
            ...
            try {
                ownerProfile = await dbProfile.fetch(branch.owner_id);
                state.enterpriseName = ownerProfile?.business_name || 'BMS Enterprise';
                state.profile = ownerProfile || { currency: 'USD' };
                if (branch.currency) {
            ...
            if (ownerProfile?.status === 'deletion_requested' || branch.is_active === false) {
...
export function _tryRestoreOfflineSession(session = null, shouldReRenderUI = true) {
    try {
        ...
        if (!cachedRaw && activeUserId) {
            for (const r of ['branch', 'owner', 'sysadmin']) {
                const candidate = localStorage.getItem(`bms_session_${r}_${activeUserId}`);
                if (candidate) {
                    cachedRaw = candidate;
                    break;
