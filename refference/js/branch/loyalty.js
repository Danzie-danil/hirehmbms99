...
window.openLoyaltyModal = async function () {
    const { data: customers } = await supabaseClient.from('customers').select('id, name, loyalty_points').eq('branch_id', state.branchId).order('name');
...
window.submitLoyaltyTransaction = async function () {
    ...
    try {
        ...
        const { error: txErr } = await supabaseClient.from('loyalty_transactions').insert({
            branch_id: state.branchId, customer_id: customerId, type, points, description
        });
        if (txErr) throw txErr;

        const { data: cust } = await supabaseClient.from('customers').select('loyalty_points').eq('id', customerId).single();
        ...
        await supabaseClient.from('customers').update({ loyalty_points: newPoints, loyalty_tier: newTier }).eq('id', customerId);
