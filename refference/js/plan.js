...
export function getPlan() {
    ...
    if (entitlements && typeof entitlements === 'object') {
        ...
        return {
            id: (entitlements.plan_id || 'free_trial').toLowerCase(),
            billingCycle: entitlements.billing_cycle || 'monthly',
            subscriptionExpiresAt: entitlements.subscription_expires_at ? new Date(entitlements.subscription_expires_at) : null,
            isActive: !!entitlements.is_active,
            isExpired: isExpired,
            isTrial: !!entitlements.is_trial,
            isTrialActive: !!entitlements.is_trial_active,
            isTrialExpired: isTrialExpired,
            isSkippedTrial: isSkippedTrial,
            isPaid: !!entitlements.is_paid,
            daysLeft: entitlements.is_trial_active && entitlements.trial_ends_at ? Math.max(0, Math.ceil((new Date(entitlements.trial_ends_at) - new Date()) / 86400000)) : 0,
            daysSinceExpiry: isTrialExpired && entitlements.trial_ends_at ? Math.max(0, Math.floor((new Date() - new Date(entitlements.trial_ends_at)) / 86400000)) : 0,
            trialEnds: entitlements.trial_ends_at ? new Date(entitlements.trial_ends_at) : null,
            maxBranches: entitlements.max_branches,
            branchCount: entitlements.branch_count || (state.branches || []).length,
            isProfileLoading: false
        };
    }

    // Default loading state before Supabase RPC returns (no date calculation/validation)
    return {
        id: 'free_trial',
