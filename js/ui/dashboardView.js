...
async function showActiveSystemBanners() {
    ...
    let banners = [];
    try {
        banners = await dbBanners.fetchActive();
    } catch (e) {
        console.error('[Banners] Failed fetching active banners:', e);
    }
...
window.handleBannerCtaAction = function(action, target, bannerId) {
    if (bannerId) {
        ...
        dbBanners.recordCtaClick(bannerId);
    }
...
export function ensureSidebarNavVisible(role) {
    ...
    if (!targetRole || !['sysadmin', 'owner', 'branch'].includes(targetRole)) {
...
export function applyDashboardRole(role) {
    ...
    const hasActiveModal = !!(state._currentModalType && state._currentModalType !== 'none');
    ...
    if (role === 'sysadmin') {
        ...
        if (!hasActiveModal && !hasPendingModalRestore) {
...
export function initDashboardView() {
    subscribe((property, value, previousValue) => {
        ...
        if (property === 'profile' && value) {
            ...
            if (ownerWelcomeEl && typeof window.getOwnerDisplayName === 'function') {
                ownerWelcomeEl.textContent = `${window.t ? window.t('welcome_back', 'Welcome back') : 'Welcome back'}, ${window.getOwnerDisplayName()}`;
            }
...
export function updateSubscriptionBadge() {
    ...
    const plan = getPlan();
    const currentPlanId = (plan.id || state.profile?.plan || (state.role === 'branch' ? state.branchProfile?.plan : '') || '').toLowerCase();

    let labelText = '';
    let iconHtml = `<i data-lucide="award" class="w-5 h-5"></i>`;

    if (currentPlanId === 'exclusive') {
        ...
        if (plan.isExpired) {
            ...
            badge.className = "inline-flex items-center justify-center cursor-help select-none transition-transform hover:scale-110 active:scale-95";
            iconHtml = `<i data-lucide="award" class="w-6 h-6 text-indigo-500"></i>`;
        }
    } else {
        labelText = currentPlanId ? currentPlanId.toUpperCase() : 'PRO';
        badge.className = "inline-flex items-center justify-center cursor-help select-none transition-transform hover:scale-110 active:scale-95";
        iconHtml = `<i data-lucide="award" class="w-5 h-5 text-indigo-500"></i>`;
    }
