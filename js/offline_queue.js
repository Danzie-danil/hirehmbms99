        ...
        try {
            ...
            switch (entityType) {
                ...
                case 'loans':
                    ...
                    break;
                case 'business_loans':
                    res = await dbBusinessLoans.create(payload);
                    break;
                case 'capital_accounts':
                    res = await dbCapital.createAccount(payload);
                    break;
                case 'business_assets':
                    res = await dbAssets.add(payload);
                    break;
                case 'asset_maintenance':
                    res = await dbAssetMaintenance.add(payload);
                    break;
                case 'attendance':
                    res = await dbAttendance.create(payload);
                    break;
                case 'shifts':
                    res = await dbShifts.startShift(payload.branch_id, payload.cashier_id, payload.opening_cash);
                    break;
                case 'promotions':
                    res = await dbPromotions.add(payload);
                    break;
                case 'goals':
                    res = await dbGoals.add(payload);
                    break;
                case 'quotations':
                    res = await dbQuotations.add(payload);
                    break;
                case 'documents':
                    res = await dbDocuments.add(payload.branch_id || state.branchId, payload);
                    break;
                case 'announcements':
                    res = await dbAnnouncements.add(payload);
                    break;
                case 'stock_transfers':
                    res = await dbStockTransfers.add(payload);
                    break;
                case 'purchase_orders':
                    res = await dbPurchaseOrders.create(payload);
                    break;
                case 'requests':
...
export async function syncOfflineSales() {
    ...
    try {
        ...
        if (!session || !session.user) {
            const { data: refreshed } = await client.auth.refreshSession().catch(() => ({ data: {} }));
    ...
    syncAllInFlight = (async () => {
        try {
            if (window.syncManager && typeof window.syncManager.processPendingQueue === 'function') {
                await window.syncManager.processPendingQueue();
            }
        } catch (smErr) {
            console.warn('[OfflineQueue] syncManager queue flush notice:', smErr);
        }
...
export function updateOfflineStatusUI() {
    // Bottom overlay removed as per UI streamlining requirements.
    // Ensure any previously created banner instance is removed.
