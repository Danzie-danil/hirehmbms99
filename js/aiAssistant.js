...
function buildClientTelemetrySummary() {
    try {
        const role = state.role;
        if (role === 'owner') {
            const branches = state.branches || [];
            const todaySales = branches.reduce((s, b) => s + (Number(b.todaySales) || 0), 0);
            const totalExp = (state.expenses || []).reduce((s, e) => s + (Number(e.amount) || 0), 0);
            const branchBreakdown = branches.map(b => `${b.name}: Today Sales TSh ${Number(b.todaySales || 0).toLocaleString()}, Target TSh ${Number(b.target || 0).toLocaleString()}`).join('; ');
            return `CLIENT CONTEXT (Real-Time State for ${state.enterpriseName || 'Business'}): Active Branches: ${branches.length} (${branchBreakdown || 'None'}), Today Gross Sales: TSh ${todaySales.toLocaleString()}, Recorded Expenses: TSh ${totalExp.toLocaleString()}`;
        } else if (role === 'branch') {
            const bProfile = state.branchProfile || {};
            const todaySales = state.todaySales || 0;
            return `CLIENT CONTEXT (Real-Time State for Branch ${bProfile.name || 'Local'}): Today Branch Sales: TSh ${Number(todaySales).toLocaleString()}`;
        }
    } catch (e) {}
    return '';
}
...
async function queryGroqAPI(prompt) {
    ...
    const telemetry = buildClientTelemetrySummary();
