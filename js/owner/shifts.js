...
import { dbBranches, dbStaff, dbCustomers, dbShifts } from '../db.js';
...
export async function renderShiftsModule() {
    ...
    try {
        ...
        const shifts = await dbShifts.fetchAll(ownerId, branchIds);
...
export async function renderAddShiftView() {
    ...
    setTimeout(() => {
        if (window.hydrateFormDraft) window.hydrateFormDraft('ownerShiftDraft', area);
        if (window.attachFormDraftAutoSave) window.attachFormDraftAutoSave('ownerShiftDraft', area);
    }, 50);
}
...
window.submitShift = async function () {
    ...
    try {
        ...
        if (window.clearFormDraft) window.clearFormDraft('ownerShiftDraft');
