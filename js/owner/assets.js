...
export async function renderOwnerAssetsModule() {
    ...
    try {
        const [assets, maintenanceLogs, branches] = await Promise.race([
            Promise.all([
            ...
            new Promise(resolve => setTimeout(() => resolve([
                window._rawAssetsList || [],
                window._rawMaintenanceLogs || [],
                state.branches || []
            ]), 12000))
        ]);
            ...
            <div class="space-y-6">
                ...
                <div class="grid grid-cols-2 lg:grid-cols-4 gap-3 sm:gap-4">
                    <div class="relative bg-white dark:bg-gray-800/90 border border-gray-200/80 dark:border-gray-700/80 rounded-2xl p-2.5 sm:p-3 shadow-xs flex flex-col justify-between h-full stat-card min-w-0">
                        ...
                        <div class="text-gray-500 dark:text-gray-400 mb-1 pr-1 sm:pr-2 min-w-0">
                            <span class="text-[11px] sm:text-xs font-bold uppercase tracking-tight sm:tracking-wider block whitespace-normal break-words leading-tight">Total Asset Valuation</span>
                        </div>
                        <p class="text-dynamic-lg font-black text-gray-900 dark:text-white leading-tight my-1 pr-1 sm:pr-2 cursor-pointer select-none" data-tooltip="${window.fmt.currency(totalAssetCost)}" data-tooltip-title="Total Asset Valuation" title="${window.fmt.currency(totalAssetCost)}">${window.fmt.number(totalAssetCost)}</p>
                        <p class="text-[10px] sm:text-[11px] text-gray-400 dark:text-gray-500 leading-tight mt-auto pr-6 truncate">Total initial purchase cost</p>
                        <div class="absolute bottom-1 right-1 w-4.5 h-4.5 rounded-[6px] bg-amber-50 dark:bg-amber-950/50 text-amber-600 dark:text-amber-400 flex items-center justify-center shadow-2xs">
                            <i data-lucide="box" class="w-2.5 h-2.5"></i>
                        </div>
                    </div>
                    ...
                    <div class="relative bg-white dark:bg-gray-800/90 border border-gray-200/80 dark:border-gray-700/80 rounded-2xl p-2.5 sm:p-3 shadow-xs flex flex-col justify-between h-full stat-card min-w-0">
                        ...
                        <div class="text-gray-500 dark:text-gray-400 mb-1 pr-1 sm:pr-2 min-w-0">
                            <span class="text-[11px] sm:text-xs font-bold uppercase tracking-tight sm:tracking-wider block whitespace-normal break-words leading-tight">Current Book Value</span>
                        </div>
                        <p class="text-dynamic-lg font-black text-emerald-600 dark:text-emerald-400 leading-tight my-1 pr-1 sm:pr-2 cursor-pointer select-none" data-tooltip="${window.fmt.currency(totalBookValue)}" data-tooltip-title="Current Book Value" title="${window.fmt.currency(totalBookValue)}">${window.fmt.number(totalBookValue)}</p>
                        <p class="text-[10px] sm:text-[11px] text-gray-400 dark:text-gray-500 leading-tight mt-auto pr-6 truncate">Post-depreciation balance</p>
                    ...
                    <div class="relative bg-white dark:bg-gray-800/90 border border-gray-200/80 dark:border-gray-700/80 rounded-2xl p-2.5 sm:p-3 shadow-xs flex flex-col justify-between h-full stat-card min-w-0">
                        ...
                        <div class="text-gray-500 dark:text-gray-400 mb-1 pr-1 sm:pr-2 min-w-0">
                            <span class="text-[11px] sm:text-xs font-bold uppercase tracking-tight sm:tracking-wider block whitespace-normal break-words leading-tight">Total Maintenance Spent</span>
                        </div>
                        <p class="text-dynamic-lg font-black text-indigo-600 dark:text-indigo-400 leading-tight my-1 pr-1 sm:pr-2 cursor-pointer select-none" data-tooltip="${window.fmt.currency(totalMaintenanceCost)}" data-tooltip-title="Total Maintenance Spent" title="${window.fmt.currency(totalMaintenanceCost)}">${window.fmt.number(totalMaintenanceCost)}</p>
                        <p class="text-[10px] sm:text-[11px] text-gray-400 dark:text-gray-500 leading-tight mt-auto pr-6 truncate">Servicing & repairs expenditure</p>
                    ...
                    <div class="relative bg-white dark:bg-gray-800/90 border border-gray-200/80 dark:border-gray-700/80 rounded-2xl p-2.5 sm:p-3 shadow-xs flex flex-col justify-between h-full stat-card min-w-0">
                        <div class="text-gray-500 dark:text-gray-400 mb-1 pr-1 sm:pr-2 min-w-0">
                            <span class="text-[11px] sm:text-xs font-bold uppercase tracking-tight sm:tracking-wider block whitespace-normal break-words leading-tight">Active vs Service</span>
                        </div>
                        <p class="text-dynamic-lg font-black text-gray-900 dark:text-white leading-tight my-1 pr-1 sm:pr-2 cursor-pointer select-none" data-tooltip="${activeAssetsCount} Active Assets (${maintenanceCount} under maintenance)" data-tooltip-title="Equipment Status">${activeAssetsCount} Active <span class="text-xs font-medium text-amber-500">(${maintenanceCount} repair)</span></p>
                        <p class="text-[10px] sm:text-[11px] text-gray-400 dark:text-gray-500 leading-tight mt-auto pr-6 truncate">Equipment operational status</p>
                ...
                <div class="bg-white dark:bg-gray-800/90 border border-gray-200/80 dark:border-gray-700/80 rounded-2xl p-4 sm:p-5 shadow-xs space-y-4">
                    <div class="flex flex-col sm:flex-row sm:items-center justify-between gap-4">
                        <div>
                            <h3 class="text-sm sm:text-base font-bold text-gray-900 dark:text-white">Fixed Assets Custom Lists</h3>
                ...
                <div class="bg-white dark:bg-gray-800/90 border border-gray-200/80 dark:border-gray-700/80 rounded-2xl p-4 sm:p-5 shadow-xs space-y-4">
                    <div class="flex items-center justify-between">
                        <div>
                            <h3 class="text-sm sm:text-base font-bold text-gray-900 dark:text-white">Dedicated Asset Servicing & Repair History</h3>
...
function renderAssetsGridHTML(filteredAssets, branchMap) {
    ...
    return filteredAssets.map(a => `
        <div class="border border-gray-200/80 dark:border-gray-700/80 rounded-2xl p-3.5 sm:p-4 bg-slate-50/50 dark:bg-gray-900/40 hover:border-amber-300 transition-all flex flex-col justify-between space-y-2.5">
            <div>
                <div class="flex items-center justify-between mb-1.5">
                    <span class="text-[10px] font-bold uppercase px-2 py-0.5 rounded-md border ${a.status === 'active' ? 'bg-emerald-50 text-emerald-700 border-emerald-200 dark:bg-emerald-950/40 dark:text-emerald-300' : a.status === 'under_maintenance' ? 'bg-amber-50 text-amber-700 border-amber-200 dark:bg-amber-950/40 dark:text-amber-300' : 'bg-red-50 text-red-700 border-red-200 dark:bg-red-950/40 dark:text-red-300'}">
                        ${a.status.replace('_', ' ')}
                    </span>
                    <span class="text-[11px] text-amber-600 dark:text-amber-400 font-bold">${a.category.toUpperCase()}</span>
                </div>
                <h4 class="font-bold text-gray-900 dark:text-white text-sm leading-snug">${a.asset_name}</h4>
                <p class="text-[11px] text-gray-400 mt-0.5">Location: ${a.branch_id ? (branchMap?.get?.(a.branch_id) || 'Branch') : 'Global'} ${a.serial_number ? '• S/N: ' + a.serial_number : ''}</p>
            </div>
...
function setupRealtimeAssetsSubscription(ownerId) {
    ...
    try {
        const handleMutation = (e) => {
            ...
            if (table === 'business_assets' || table === 'asset_maintenance_logs') {
        ...
        console.warn('[OwnerAssets] Realtime listener error:', e);
    }
}
...
export async function renderAddAssetView(editAssetId = null) {
    ...
    if (!editAssetId) {
        setTimeout(() => {
            if (window.hydrateFormDraft) window.hydrateFormDraft('ownerAssetDraft', area);
            if (window.attachFormDraftAutoSave) window.attachFormDraftAutoSave('ownerAssetDraft', area);
        }, 50);
    }
}
...
window.handleSaveAssetBatch = async function(editAssetId = null) {
    ...
    try {
        ...
        window._batchAssetsList = [];
        if (window.clearFormDraft) window.clearFormDraft('ownerAssetDraft');
        window.renderOwnerAssetsModule();
    } catch (err) {
...
export async function renderAddMaintenanceView(preselectAssetId = null) {
    ...
    setTimeout(() => {
        if (window.hydrateFormDraft) window.hydrateFormDraft('ownerAssetMaintenanceDraft', area);
        if (window.attachFormDraftAutoSave) window.attachFormDraftAutoSave('ownerAssetMaintenanceDraft', area);
    }, 50);
}
...
window.handleSaveMaintenance = async function() {
    ...
    try {
        ...
        if (window.clearFormDraft) window.clearFormDraft('ownerAssetMaintenanceDraft');
...
window.renderBatchAssetsPreview = window.updateBatchAssetsUI;
