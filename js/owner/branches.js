...
export async function renderBranchesManagement() {
    ...
    const renderDOM = async (branches) => {
        ...
        <div class="space-y-4 slide-in">
            ...
            <div class="grid grid-cols-2 sm:grid-cols-2 md:grid-cols-4 gap-3 sm:gap-4 pt-2">
                <div data-tooltip="Total number of registered branches under your business" data-tooltip-title="Branch Count" class="bg-white dark:bg-gray-800 px-3 py-2.5 rounded-2xl border border-gray-100 dark:border-gray-700 shadow-sm stat-card min-w-0 flex flex-col justify-between h-full">
                    <p class="text-[11px] sm:text-xs text-gray-500 dark:text-gray-400 uppercase tracking-tight whitespace-normal break-words font-bold leading-tight" title="${totalBranchesText}">${totalBranchesText}</p>
    ...
    // 1. FAST PATH: Check memory and local cache for instant render
    let cachedBranches = Array.isArray(state.branches) && state.branches.length > 0 ? state.branches : null;
    if (!cachedBranches && ownerId) {
        try {
            const cachedStr = localStorage.getItem(`bms_cached_branches_${ownerId}`);
            if (cachedStr) cachedBranches = JSON.parse(cachedStr);
        } catch (e) {}
    }

    if (cachedBranches && cachedBranches.length > 0) {
        await renderDOM(cachedBranches);
    } else {
        container.innerHTML = `
        <div class="space-y-4 slide-in">
            <div class="flex flex-nowrap items-center gap-2 sm:gap-3 justify-between">
    ...
    // 2. BACKGROUND FETCH: Silently update with fresh data
    dbBranches.fetchAll(ownerId).then(fresh => {
        if (Array.isArray(fresh)) {
            renderDOM(fresh);
        }
    }).catch(err => {
        console.warn('[OwnerBranches] Background branch fetch notice:', err.message);
        if (!cachedBranches || cachedBranches.length === 0) {
...
export async function deleteBranchRow(id, name) {
    const confirmed = await confirmModal(
        'Delete Branch',
        `Are you absolutely sure you want to delete the branch "${name}"? This action cannot be undone and will permanently remove this branch and all its associated data (inventory, sales, expenses, etc.).`,
        'Delete Branch',
        'Cancel',
    ...
    try {
        if (typeof window.showLoader === 'function') window.showLoader(`Deleting branch "${name}"...`);

        const rpcResult = await dbBranches.delete(id);
        ...
        if (rpcResult && rpcResult.reassigned_units > 0) {
            showToast(`Branch "${name}" deleted. ${rpcResult.reassigned_units} units reassigned back to Main Store.`, 'success');
        } else {
        ...
        showToast(`Failed to delete branch: ${err.message || err}`, 'error');
    }
};
