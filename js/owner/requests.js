...
import { dbRequests, dbInventory, dbInventoryPurchases, dbSales, dbExpenses, dbCustomers, dbBusinessLoans } from '../db.js';
import { renderPremiumLoader, fmt, showToast, confirmModal, promptModal } from '../utils.js';
...
export async function renderRequestsList(highlightId = null) {
    ...
    try {
        ...
        listContainer.innerHTML = requests.map(req => {
            ...
            // ── Special rich card: Sale Approval Request ──
            if (req.type === 'sales_add' && req.metadata) {
                ...
                <div id="req-${req.id}" class="bg-white dark:bg-gray-800 border border-gray-200 dark:border-gray-700 border-l-[4px] ${statusColors[req.status]} rounded-2xl p-5 transition-all ${isHighlighted ? 'ring-2 ring-indigo-500 shadow-lg' : 'hover:shadow-md'} relative group">
                    <div class="flex items-start justify-between gap-4 mb-4">
                        <div class="flex items-center gap-3">
                            ...
                            <div>
                                ...
                                <p class="text-[10px] text-gray-500 dark:text-gray-400 font-medium uppercase tracking-wider">${req.branches?.name || 'Branch'} · ${fmt.dateTime(req.created_at)}</p>
                            </div>
                        </div>
                    ...
                    <div class="grid grid-cols-3 gap-2 mb-4">
                        <div class="bg-gray-50 dark:bg-gray-700/50 p-3 rounded-xl">
                            ...
                            <p class="font-black text-gray-900 dark:text-white text-sm truncate">${m.customer || 'Walk-in'}</p>
                        </div>
                        <div class="bg-gray-50 dark:bg-gray-700/50 p-3 rounded-xl">
                            <p class="text-[10px] text-gray-400 font-bold uppercase">Payment</p>
                            <p class="font-black text-indigo-600 dark:text-indigo-400 text-sm uppercase">${m.payment || 'Cash'}</p>
                        </div>
                        <div class="bg-gray-50 dark:bg-gray-700/50 p-3 rounded-xl">
                            <p class="text-[10px] text-gray-400 font-bold uppercase">Sale Total</p>
                            <p class="font-black text-emerald-600 dark:text-emerald-400 text-sm">${fmt.currency(m.amount || 0)}</p>
                        </div>
                    </div>
                    <div class="p-3 bg-gray-50 dark:bg-gray-700/50 rounded-xl mb-4 text-xs text-gray-700 dark:text-gray-300 flex items-center justify-between gap-2">
                        <div><span class="font-bold text-gray-500">Items: </span>${m.items || 'Direct Sale Items'}</div>
                        ${(m.item_type === 'service' || (typeof m.items === 'string' && m.items.toLowerCase().includes('service'))) ? `
                        <span class="px-2 py-0.5 rounded-lg text-[10px] font-black uppercase tracking-wider bg-purple-50 dark:bg-purple-950/50 text-purple-700 dark:text-purple-300 border border-purple-200/60 shrink-0">
                            🛠️ Service Offering
                        </span>` : ''}
                    </div>
                    ${req.status === 'pending' ? `
                    <div class="flex gap-2 pt-2 border-t border-gray-100 dark:border-gray-700 mt-2">
                        <button onclick="handleRequestAction('${req.id}', 'approved')" class="flex-1 py-2 bg-emerald-600 text-white rounded-xl text-xs font-black uppercase hover:bg-emerald-700 transition-all flex items-center justify-center gap-1.5 shadow-sm">
                            <i data-lucide="check-circle" class="w-3.5 h-3.5"></i> Approve Sale
                        </button>
            ...
            // ── Special rich card: Expense Approval Request ──
            if (req.type === 'expenses_add' && req.metadata) {
                const m = req.metadata;
                return `
                <div id="req-${req.id}" class="bg-white dark:bg-gray-800 border border-gray-200 dark:border-gray-700 border-l-[4px] ${statusColors[req.status]} rounded-2xl p-5 transition-all ${isHighlighted ? 'ring-2 ring-indigo-500 shadow-lg' : 'hover:shadow-md'} relative group">
                    <div class="flex items-start justify-between gap-4 mb-4">
                        <div class="flex items-center gap-3">
                            <div class="w-10 h-10 bg-rose-100 dark:bg-rose-900/30 rounded-xl flex items-center justify-center flex-shrink-0">
                                <i data-lucide="receipt" class="w-5 h-5 text-rose-600 dark:text-rose-400"></i>
                            </div>
                            <div>
                                <h4 class="font-bold text-gray-900 dark:text-white leading-tight">${req.subject}</h4>
                                <p class="text-[10px] text-gray-500 dark:text-gray-400 font-medium uppercase tracking-wider">${req.branches?.name || 'Branch'} · ${fmt.dateTime(req.created_at)}</p>
                    ...
                    <div class="grid grid-cols-2 gap-2 mb-4">
                        <div class="bg-gray-50 dark:bg-gray-700/50 p-3 rounded-xl">
                            ...
                            <p class="font-black text-gray-900 dark:text-white text-sm uppercase">${m.category || 'General'}</p>
                        </div>
                        <div class="bg-gray-50 dark:bg-gray-700/50 p-3 rounded-xl">
                            <p class="text-[10px] text-gray-400 font-bold uppercase">Expense Amount</p>
                            <p class="font-black text-rose-600 dark:text-rose-400 text-sm">${fmt.currency(m.amount || 0)}</p>
                        </div>
                    </div>
                    <div class="p-3 bg-gray-50 dark:bg-gray-700/50 rounded-xl mb-4 text-xs text-gray-700 dark:text-gray-300">
                        <span class="font-bold text-gray-500">Description: </span>${m.description || 'No description provided'}
                    </div>
                    ${req.status === 'pending' ? `
                    <div class="flex gap-2 pt-2 border-t border-gray-100 dark:border-gray-700 mt-2">
                        <button onclick="handleRequestAction('${req.id}', 'approved')" class="flex-1 py-2 bg-emerald-600 text-white rounded-xl text-xs font-black uppercase hover:bg-emerald-700 transition-all flex items-center justify-center gap-1.5 shadow-sm">
                            <i data-lucide="check-circle" class="w-3.5 h-3.5"></i> Approve Expense
                        </button>
                        <button onclick="handleRequestAction('${req.id}', 'rejected')" class="px-4 py-2 bg-white dark:bg-gray-700 border border-gray-200 dark:border-gray-600 text-red-600 dark:text-red-400 rounded-xl text-xs font-bold hover:bg-red-50 dark:hover:bg-red-900/20 transition-all flex items-center justify-center gap-1.5">
                            <i data-lucide="x-circle" class="w-3.5 h-3.5"></i> Reject
                        </button>
            ...
            // ── Special rich card: Service Addition Request ──
            if (req.type === 'inventory_add' && req.metadata && (req.metadata.item_type === 'service' || (req.subject && req.subject.toLowerCase().includes('service')))) {
                const m = req.metadata;
                return `
                <div id="req-${req.id}" class="bg-white dark:bg-gray-800 border border-gray-200 dark:border-gray-700 border-l-[4px] ${statusColors[req.status]} rounded-2xl p-5 md:p-6 transition-all ${isHighlighted ? 'ring-2 ring-indigo-500 shadow-lg' : 'hover:shadow-md'} relative group">
                    <div class="flex items-start justify-between gap-4 mb-4">
                        <div class="flex items-center gap-3">
                            <div class="w-10 h-10 bg-indigo-100 dark:bg-indigo-900/30 text-indigo-600 dark:text-indigo-400 rounded-xl flex items-center justify-center flex-shrink-0">
                    ...
                    <div class="grid grid-cols-2 sm:grid-cols-3 gap-2.5 mb-4">
                        <div class="bg-indigo-50/50 dark:bg-indigo-950/20 p-3 rounded-xl border border-indigo-100/50 dark:border-indigo-900/30">
                            <p class="text-[10px] text-indigo-600 dark:text-indigo-400 font-bold uppercase">Service Price</p>
                            <p class="font-black text-indigo-700 dark:text-indigo-300 text-sm">${fmt.currency(m.price || m.retail_price || 0)}</p>
                        </div>
                        <div class="bg-amber-50/50 dark:bg-amber-950/20 p-3 rounded-xl border border-amber-100/50 dark:border-amber-900/30">
                            <p class="text-[10px] text-amber-600 dark:text-amber-400 font-bold uppercase">Service Cost</p>
                            <p class="font-black text-amber-700 dark:text-amber-300 text-sm">${fmt.currency(m.cost_price || 0)}</p>
                        </div>
                        <div class="col-span-2 sm:col-span-1 bg-gray-50 dark:bg-gray-700/50 p-3 rounded-xl border border-gray-100 dark:border-gray-700/50">
                            <p class="text-[10px] text-gray-400 font-bold uppercase">Category</p>
                            <p class="font-black text-gray-900 dark:text-white text-sm truncate">${m.category || 'Services'}</p>
                        </div>
                    </div>
                    ...
                    <div class="flex flex-wrap gap-2 pt-2 border-t border-gray-100 dark:border-gray-700 mt-4">
                        <button onclick="handleRequestAction('${req.id}', 'approved')" class="flex-1 py-2.5 bg-indigo-600 text-white rounded-xl text-xs font-black uppercase tracking-widest hover:bg-indigo-700 transition-all flex items-center justify-center gap-2 shadow-lg shadow-indigo-100 dark:shadow-none">
                            <i data-lucide="check-circle" class="w-4 h-4"></i> Approve Service
                        </button>
                ...
                <div class="flex items-start justify-between gap-4 mb-4">
                    <div class="flex items-center gap-3">
                        <div class="w-10 h-10 ${badgeColors[req.status]} rounded-xl flex items-center justify-center flex-shrink-0">
                            <i data-lucide="${req.type?.includes('inventory') ? 'package' : req.type?.includes('sale') ? 'shopping-cart' : req.type?.includes('expense') ? 'receipt' : 'message-square'}" class="w-5 h-5"></i>
                        </div>
                        <div>
                            <h4 class="font-bold text-gray-900 dark:text-white leading-tight">${req.subject}</h4>
                            <p class="text-[10px] text-gray-500 dark:text-gray-400 font-medium uppercase tracking-wider">${req.branches?.name || 'Unknown Branch'} · ${fmt.dateTime(req.created_at)}</p>
...
export async function handleRequestAction(id, status) {
    ...
    try {
        ...
        if (status === 'approved') {
            if (req.type === 'sales_add' && req.metadata) {
                await dbSales.add(req.branch_id, req.metadata, req.metadata.client_tx_id || crypto.randomUUID());
                const branchName = req.branches?.name || 'Branch';
                if (typeof window.addActivity === 'function') {
                    window.addActivity('sale', `Approved sale to ${req.metadata.customer || 'Customer'}`, branchName, req.metadata.amount);
                }
            } else if (req.type === 'expenses_add' && req.metadata) {
                await dbExpenses.add(req.branch_id, req.metadata);
                const branchName = req.branches?.name || 'Branch';
                if (typeof window.addActivity === 'function') {
                    window.addActivity('expense', `Approved expense: ${req.metadata.description || req.metadata.category}`, branchName, req.metadata.amount);
                }
            } else if (req.type === 'customers_add' && req.metadata) {
                await dbCustomers.add(req.branch_id, req.metadata);
            } else if (req.type === 'inventory_add' && req.metadata) {
                const isService = req.metadata.item_type === 'service' || (req.subject && req.subject.toLowerCase().includes('service'));
                const { name, sku, category, price, quantity, min_threshold, supplier, cost_price, retail_price, wholesale_price } = req.metadata;

                const itemPayload = {
                    name,
                    sku: sku || (isService ? (typeof window.generateAutoSKU === 'function' ? window.generateAutoSKU('Services', name) : `SVC-${Date.now().toString().slice(-6)}`) : `PRD-${Date.now().toString().slice(-6)}`),
                    category: category || (isService ? 'Services' : 'General'),
                    price: price || retail_price || 0,
                    retail_price: retail_price || price || 0,
                    wholesale_price: wholesale_price || price || 0,
                    cost_price: cost_price || 0,
                    quantity: isService ? 0 : (quantity || 0),
                    min_threshold: isService ? 0 : (min_threshold || 10),
                    item_type: isService ? 'service' : 'product',
                    is_isolated: false,
                    isolation_status: 'unregistered'
                };

                const addedItem = await dbInventory.add(req.branch_id, itemPayload);

                // Only record inventory purchase entry for physical stock items, NOT for services
                if (!isService && (quantity || 0) > 0) {
                ...
                if (typeof window.addActivity === 'function') {
                    window.addActivity(
                        ...
                        `Approved ${isService ? 'service' : 'new stock'}: ${name}`,
                        branchName,
                        isService ? (price || retail_price || 0) : ((quantity || 0) * (cost_price || 0))
                    );
                }
