    ...
    try {
        ...
        if (branchIds.length > 0) {
            ...
            const res = await Promise.race([
                req,
                new Promise((_, reject) => setTimeout(() => reject(new Error('tasks timeout')), 12000))
            ]);

            const tasks = res?.data;
            if (!res?.error && Array.isArray(tasks)) {
                myTasks = tasks.map(t => ({
                    ...t,
        ...
        console.warn('[OwnerTasks] Remote fetch notice, loading from local storage:', err.message);
        try {
            myTasks = await dbTasks.fetchByOwner(ownerId);
        } catch (dbErr) {
            myTasks = [];
