...
export async function renderOwnerStaffModule() {
    ...
    try {
        const [branches, allStaff] = await Promise.race([
            (async () => {
                ...
                if (!sList || sList.length === 0) {
                    ...
                    sList = staffLists.flat();
                } else {
                    sList = sList.map(s => ({
                        ...s,
                        _branchName: bMap.get(s.branch_id) || 'Unassigned Branch',
                        _branchId: s.branch_id
                    }));
                }
                return [bList, sList];
            })(),
            new Promise(resolve => setTimeout(() => resolve([state.branches || [], window.currentAllStaff || []]), 12000))
        ]);
        ...
        <div class="space-y-4 sm:space-y-5 slide-in">
            ...
            <div class="bg-white dark:bg-gray-800 rounded-2xl border border-gray-200 dark:border-gray-700 p-3.5 sm:p-5 flex flex-col sm:flex-row sm:items-center sm:justify-between gap-3">
                <div class="flex items-center gap-2.5 sm:gap-3 min-w-0 flex-1">
                    ...
                    <div class="min-w-0 flex-1">
                        <h2 class="text-sm sm:text-lg font-black text-gray-900 dark:text-white tracking-tight truncate">${window.t('staff_hr_monitor', 'Staff & HR Monitor')}</h2>
                ...
                <div class="flex items-center gap-1.5 sm:gap-2 w-full sm:w-auto">
                    <button onclick="window.renderCustomRoleMatrix()" class="flex-1 sm:flex-initial px-2.5 sm:px-3 py-1.5 sm:py-2 bg-gray-50 dark:bg-gray-700/60 border border-gray-200 dark:border-gray-700 rounded-xl text-[11px] sm:text-xs font-bold text-gray-700 dark:text-gray-300 hover:bg-indigo-50 dark:hover:bg-indigo-950/30 hover:text-indigo-600 transition-colors flex items-center justify-center gap-1.5 cursor-pointer whitespace-nowrap">
                        <i data-lucide="shield-check" class="w-3 h-3 sm:w-3.5 sm:h-3.5 text-indigo-500"></i>
                        <span>Role Matrix</span>
                    </button>
                    <button onclick="window.openOwnerAddStaffModal()" class="flex-1 sm:flex-initial inline-flex items-center justify-center gap-1.5 px-3 sm:px-3.5 py-1.5 sm:py-2 rounded-xl text-[11px] sm:text-xs font-bold text-white bg-indigo-600 hover:bg-indigo-700 transition-all cursor-pointer shadow-xs whitespace-nowrap">
                        <i data-lucide="user-plus" class="w-3 h-3 sm:w-3.5 sm:h-3.5"></i>
                        <span>${window.t('add_staff', 'Add Staff')}</span>
                    </button>
                </div>
            </div>
            ...
            <div class="grid grid-cols-2 sm:grid-cols-2 lg:grid-cols-4 gap-2.5 sm:gap-3.5">
                ...
                <div class="relative bg-white dark:bg-gray-800 p-3 sm:p-4 rounded-2xl border border-gray-200 dark:border-gray-700 stat-card flex flex-col justify-between h-full min-w-0">
                    <div class="flex items-center justify-between mb-1.5 pr-10">
                        <span class="text-[11px] font-bold text-gray-500 dark:text-gray-400 uppercase tracking-tight truncate block">${window.t('total_staff', 'Total Staff')}</span>
                    </div>
                    <div class="min-w-0 mt-auto pr-9">
                        <p class="text-dynamic-lg font-black text-gray-900 dark:text-white truncate leading-tight">${total}</p>
                        <p class="text-[10px] text-indigo-600 dark:text-indigo-400 font-semibold mt-0.5 truncate">Global Headcount</p>
                    </div>
                    <svg class="absolute bottom-2 right-2 w-5 h-3 text-indigo-400 opacity-80" viewBox="0 0 36 24" fill="currentColor">
                ...
                <div class="relative bg-white dark:bg-gray-800 p-3 sm:p-4 rounded-2xl border border-gray-200 dark:border-gray-700 stat-card flex flex-col justify-between h-full min-w-0">
                    <div class="flex items-center justify-between mb-1.5 pr-10">
                        <span class="text-[11px] font-bold text-emerald-600 dark:text-emerald-400 uppercase tracking-tight truncate block">${window.t('active', 'Active')}</span>
                    </div>
                    <div class="min-w-0 mt-auto pr-9">
                        <p class="text-dynamic-lg font-black text-emerald-600 dark:text-emerald-400 truncate leading-tight">${active}</p>
                        <p class="text-[10px] text-emerald-600 dark:text-emerald-400 font-semibold mt-0.5 truncate">On Active Duty</p>
                    </div>
                ...
                <div class="relative bg-white dark:bg-gray-800 p-3 sm:p-4 rounded-2xl border border-gray-200 dark:border-gray-700 stat-card flex flex-col justify-between h-full min-w-0">
                    <div class="flex items-center justify-between mb-1.5 pr-10">
                        <span class="text-[11px] font-bold text-amber-600 dark:text-amber-400 uppercase tracking-tight truncate block">Suspended</span>
                    </div>
                    <div class="min-w-0 mt-auto pr-9">
                        <p class="text-dynamic-lg font-black text-amber-600 dark:text-amber-400 truncate leading-tight">${suspended}</p>
                        <p class="text-[10px] text-amber-600 dark:text-amber-400 font-semibold mt-0.5 truncate">Pending Review</p>
                    </div>
                ...
                <div class="relative bg-white dark:bg-gray-800 p-3 sm:p-4 rounded-2xl border border-gray-200 dark:border-gray-700 stat-card flex flex-col justify-between h-full min-w-0">
                    <div class="flex items-center justify-between mb-1.5 pr-10">
                        <span class="text-[11px] font-bold text-red-600 dark:text-red-400 uppercase tracking-tight truncate block">Inactive / Offboarded</span>
                    </div>
                    <div class="min-w-0 mt-auto pr-9">
                        <p class="text-dynamic-lg font-black text-red-600 dark:text-red-400 truncate leading-tight">${terminated + laidOff}</p>
                        <p class="text-[10px] text-red-600 dark:text-red-400 font-semibold mt-0.5 truncate">Offboarded</p>
                    </div>
            ...
            <!-- Bento Container for Filter Pills & List -->
            <div class="bg-white dark:bg-gray-800 rounded-2xl border border-gray-200 dark:border-gray-700 p-4 sm:p-5 space-y-3.5">
                <div class="flex items-center justify-between">
                    <div>
                        <h3 class="text-xs sm:text-sm font-bold text-gray-900 dark:text-white uppercase tracking-wider">Employee Directory</h3>
                        <p class="text-[11px] text-gray-400 font-medium">Branch assignments, roles and compensation</p>
                    </div>
                </div>
                ...
                <div class="flex items-center gap-2 overflow-x-auto pb-1 scroller-custom">
                    ...
                    <button onclick="window.filterOwnerStaff('active')" data-status="active" class="staff-filter-btn px-3 py-1.5 rounded-xl font-bold text-xs transition-all cursor-pointer bg-gray-50 dark:bg-gray-700 text-gray-600 dark:text-gray-400 hover:bg-gray-100 dark:hover:bg-gray-600 border border-gray-200 dark:border-gray-600">
                        Active (${active})
                    </button>
                    <button onclick="window.filterOwnerStaff('suspended')" data-status="suspended" class="staff-filter-btn px-3 py-1.5 rounded-xl font-bold text-xs transition-all cursor-pointer bg-gray-50 dark:bg-gray-700 text-gray-600 dark:text-gray-400 hover:bg-gray-100 dark:hover:bg-gray-600 border border-gray-200 dark:border-gray-600">
                        Suspended (${suspended})
                    </button>
                    <button onclick="window.filterOwnerStaff('laid_off')" data-status="laid_off" class="staff-filter-btn px-3 py-1.5 rounded-xl font-bold text-xs transition-all cursor-pointer bg-gray-50 dark:bg-gray-700 text-gray-600 dark:text-gray-400 hover:bg-gray-100 dark:hover:bg-gray-600 border border-gray-200 dark:border-gray-600">
                        Laid Off (${laidOff})
                    </button>
                    <button onclick="window.filterOwnerStaff('terminated')" data-status="terminated" class="staff-filter-btn px-3 py-1.5 rounded-xl font-bold text-xs transition-all cursor-pointer bg-gray-50 dark:bg-gray-700 text-gray-600 dark:text-gray-400 hover:bg-gray-100 dark:hover:bg-gray-600 border border-gray-200 dark:border-gray-600">
                        Terminated (${terminated})
                    </button>
                </div>

                <!-- Staff List Container -->
                <div id="ownerStaffContainer" class="space-y-2.5">
...
window.getStaffStatusBadgeHtml = function(status) {
    ...
    return `<span class="bg-gray-100 dark:bg-gray-800 text-gray-600 dark:text-gray-400 text-[10px] px-2.5 py-0.5 rounded-lg font-black uppercase border border-gray-200 dark:border-gray-700">${status || 'Inactive'}</span>`;
};
    ...
    if (!staffList || staffList.length === 0) {
        return `
            <div class="py-12 text-center border-2 border-dashed border-gray-200 dark:border-gray-700 rounded-2xl">
                ...
                <p class="text-gray-400 text-xs font-medium">${window.t('no_staff_records', 'No staff records found')}</p>
                <button onclick="window.openOwnerAddStaffModal()" class="mt-3 px-3.5 py-1.5 bg-indigo-600 hover:bg-indigo-700 text-white font-bold rounded-xl text-xs transition-all flex items-center gap-1.5 mx-auto cursor-pointer">
                    <i data-lucide="plus" class="w-3.5 h-3.5"></i> ${window.t('add_staff', 'Add Staff Member')}
                </button>
            </div>`;
    }

    return staffList.map(s => {
        const initials = (s.name || 'ST').split(' ').map(n => n[0]).join('').substring(0, 2).toUpperCase() || 'ST';
        return `
        <div class="bg-gray-50/70 dark:bg-gray-900/50 hover:bg-white dark:hover:bg-gray-800 border border-gray-200 dark:border-gray-700 rounded-xl p-3 sm:p-4 transition-colors flex flex-col sm:flex-row sm:items-center sm:justify-between gap-3">
            <div class="flex items-center gap-3 min-w-0 flex-1">
                <div class="w-9 h-9 rounded-xl bg-indigo-50 dark:bg-indigo-900/40 text-indigo-600 dark:text-indigo-400 font-bold text-xs flex items-center justify-center flex-shrink-0 border border-indigo-100 dark:border-indigo-800">
                    ${initials}
                </div>
                <div class="min-w-0 flex-1">
                    <div class="flex items-center gap-2 flex-wrap">
                        <h4 class="font-bold text-gray-900 dark:text-white text-xs sm:text-sm leading-tight truncate">${s.name || 'Unnamed'}</h4>
            ...
            <div class="flex items-center gap-1.5 shrink-0 self-end sm:self-auto flex-wrap">
                <button onclick="window.openHRActionModal('${s.id}')" class="px-2.5 py-1.5 bg-amber-50 hover:bg-amber-100 dark:bg-amber-950/30 text-amber-700 dark:text-amber-300 font-bold rounded-lg text-xs border border-amber-200 dark:border-amber-800 transition-colors flex items-center gap-1 cursor-pointer">
                    <i data-lucide="shield-alert" class="w-3.5 h-3.5"></i> HR Actions
                </button>
                <button onclick="window.openOwnerAddStaffModal('${s.id}')" class="px-2.5 py-1.5 bg-gray-100 hover:bg-gray-200 dark:bg-gray-700 text-gray-700 dark:text-gray-200 font-bold rounded-lg text-xs transition-colors flex items-center gap-1 cursor-pointer">
                    <i data-lucide="edit-3" class="w-3.5 h-3.5"></i> Edit
                </button>
                <button onclick="window.showStaffDetailModal('${s.id}')" class="px-2.5 py-1.5 bg-indigo-50 dark:bg-indigo-950/40 text-indigo-600 dark:text-indigo-400 font-bold rounded-lg text-xs border border-indigo-100 dark:border-indigo-900 transition-colors flex items-center gap-1 cursor-pointer">
...
window.openOwnerAddStaffModal = async function(staffId = null) {
    ...
    openModal(modalHtml, null, false, isEdit ? 'ownerEditStaff' : 'ownerAddStaff');
    if (window.lucide) window.lucide.createIcons();
};
...
window.handleSaveOwnerStaff = async function(event, staffId = null) {
    ...
    try {
        if (staffId) {
            ...
            if (window.clearFormDraft) window.clearFormDraft('ownerAddStaff');
