...
import { dbBranches, dbPayroll } from '../db.js';
    ...
    try {
        ...
        <div class="space-y-5 slide-in max-w-7xl mx-auto pb-8">
            ...
            <div class="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-3.5">
                <div class="relative bg-gradient-to-br from-slate-900 via-indigo-950 to-slate-900 text-white p-4 sm:p-5 rounded-2xl shadow-sm border border-slate-800 flex flex-col justify-between">
                    <div class="absolute -top-2.5 right-3.5 px-2 py-0.5 rounded-full text-[9px] font-black uppercase tracking-wider bg-slate-900 text-amber-300 border border-slate-700 shadow-xs z-10">${fmt.getSymbol ? fmt.getSymbol() : 'TSh'}</div>
                    ...
                    <div class="mt-3">
                        <p class="text-2xl sm:text-3xl font-black text-amber-400" title="${fmt.currency(totalPending)}">${fmt.number(totalPending)}</p>
                        <p class="text-[11px] text-indigo-200/70 mt-1">Awaiting disbursement approval</p>
                    </div>
                </div>

                <div class="relative bg-white dark:bg-gray-900 p-4 sm:p-5 rounded-2xl border border-gray-100 dark:border-gray-800 shadow-xs flex flex-col justify-between">
                    ...
                    <div class="mt-3">
                        <p class="text-2xl sm:text-3xl font-black text-emerald-600 dark:text-emerald-400" title="${fmt.currency(totalPaid)}">${fmt.number(totalPaid)}</p>
...
export async function renderAddPayrollView() {
    ...
    setTimeout(() => {
        if (window.hydrateFormDraft) window.hydrateFormDraft('ownerPayrollDraft', area);
        if (window.attachFormDraftAutoSave) window.attachFormDraftAutoSave('ownerPayrollDraft', area);
    }, 50);
}
...
window.submitPayrollEntry = async function () {
    ...
    try {
        ...
        await dbPayroll.create(payload);

        if (window.clearFormDraft) window.clearFormDraft('ownerPayrollDraft');
