...
export function renderSettings() {
    ...
    const cleanMobile = (profile.mobile_number && !profile.mobile_number.includes('@')) 
        ? profile.mobile_number 
        : (profile.phone && !profile.phone.includes('@') ? profile.phone : '');

    const cleanFullName = profile.full_name || '';
    ...
    <div class="max-w-7xl mx-auto w-full opacity-0 translate-y-4 animate-fade-in-up space-y-4">
        ...
        <div class="w-full">
            <div id="settingsForm" class="bg-white dark:bg-gray-800 rounded-2xl shadow-sm border border-gray-100 dark:border-gray-700/60">

                    <!-- Tab Content: Personal Profile -->
                    <div id="content-personal" class="${activeTab === 'personal' ? 'block' : 'hidden'} p-8">
                        <h3 class="text-lg font-bold text-gray-900 dark:text-white mb-6">${window.t('personal_admin_info', 'Personal Admin Information')}</h3>
                        <div class="grid grid-cols-1 md:grid-cols-2 gap-6">
                            <div class="col-span-1 md:col-span-2">
                                <label for="set_full_name" class="block text-sm font-medium text-gray-700 dark:text-gray-300 mb-1">${window.t('full_name', 'Full Name')}</label>
                                <div class="relative w-full">
                                    <input type="text" id="set_full_name" name="bms_usr_display_name" autocomplete="off" autocorrect="off" autocapitalize="off" data-lpignore="true" data-1p-ignore="true" data-bwignore="true" data-dashlane-ignore="true" data-private="true" data-form-type="other" spellcheck="false" value="${cleanFullName}" class="form-input w-full" placeholder="e.g. John Doe">
                                </div>
                            </div>
                            <div class="col-span-1">
                                <label class="block text-sm font-medium text-gray-700 dark:text-gray-300 mb-1">${window.t('admin_email', 'Admin Email Address')}</label>
                                <div class="form-input w-full bg-gray-50 dark:bg-gray-700/50 text-gray-600 dark:text-gray-300 select-none flex items-center h-[42px] cursor-not-allowed border border-gray-200 dark:border-gray-700 rounded-xl px-3.5">
                                    <span class="truncate text-xs sm:text-sm font-semibold">${profile.email || state.currentUser || ''}</span>
                                </div>
                                <p class="text-xs text-gray-400 mt-1">${window.t('primary_comm_email', 'Primary communication email.')}</p>
                            </div>
                            <div class="col-span-1">
                                <label for="set_mobile_number" class="block text-sm font-medium text-gray-700 dark:text-gray-300 mb-1">${window.t('mobile_number', 'Mobile Number')}</label>
                                <div class="relative w-full">
                                    <input type="text" id="set_mobile_number" name="bms_usr_contact_phone" autocomplete="off" autocorrect="off" autocapitalize="off" data-lpignore="true" data-1p-ignore="true" data-bwignore="true" data-dashlane-ignore="true" data-private="true" data-form-type="other" spellcheck="false" inputmode="tel" value="${cleanMobile}" class="form-input w-full" placeholder="+255 700 000 000">
                                </div>
                            </div>
                    ...
                    <div id="content-security" class="${activeTab === 'security' ? 'block' : 'hidden'} p-8">
                        ...
                        <div class="mb-8 space-y-4">
                            ...
                            <div class="p-4 sm:p-5 bg-gray-50 dark:bg-gray-750 rounded-2xl border border-gray-100 dark:border-gray-700 space-y-4 overflow-hidden">
                                ...
                                <!-- Collapsible In-App Password Update Form Container (Dynamic Mounting) -->
                                <div id="passwordChangeFormContainer" class="hidden pt-4 border-t border-gray-200/80 dark:border-gray-700/80 space-y-4"></div>
                            </div>
                        </div>
...
export async function saveSettings() {
    ...
    ownerAutoSaveTimeout = setTimeout(async () => {
        ...
        let rawFullName = document.getElementById('set_full_name') ? document.getElementById('set_full_name').value.trim() : (state.profile?.full_name || '');
        let rawMobile = document.getElementById('set_mobile_number') ? document.getElementById('set_mobile_number').value.trim() : (state.profile?.mobile_number || '');

        // If browser autofill erroneously put an email into the mobile number field, strip/sanitize it
        if (rawMobile.includes('@')) {
            rawMobile = (state.profile?.mobile_number && !state.profile.mobile_number.includes('@')) ? state.profile.mobile_number : '';
        }

        const personal = {
            full_name: rawFullName,
            mobile_number: rawMobile,
            avatar_url: document.getElementById('set_avatar_url') ? document.getElementById('set_avatar_url').value.trim() : (state.profile?.avatar_url || '')
        };

        const business = {
            business_name: document.getElementById('set_business_name') ? document.getElementById('set_business_name').value.trim() : (state.profile?.business_name || ''),
            industry: document.getElementById('set_industry') ? document.getElementById('set_industry').value : (state.profile?.industry || ''),
            tax_id: document.getElementById('set_tax_id') ? document.getElementById('set_tax_id').value.trim() : (state.profile?.tax_id || ''),
            currency: document.getElementById('set_currency') ? document.getElementById('set_currency').value : (state.profile?.currency || 'TZS'),
            timezone: document.getElementById('set_timezone') ? document.getElementById('set_timezone').value : (state.profile?.timezone || 'Africa/Dar_es_Salaam'),
            street_address: document.getElementById('set_street_address') ? document.getElementById('set_street_address').value.trim() : (state.profile?.street_address || ''),
            city: document.getElementById('set_city') ? document.getElementById('set_city').value.trim() : (state.profile?.city || ''),
            zip_code: document.getElementById('set_zip_code') ? document.getElementById('set_zip_code').value.trim() : (state.profile?.zip_code || ''),
            brand_color: document.getElementById('set_brand_color') ? document.getElementById('set_brand_color').value.trim() : (state.profile?.brand_color || ''),
            logo_url: document.getElementById('set_logo_url') ? document.getElementById('set_logo_url').value.trim() : (state.profile?.logo_url || '')
        };

        const prefs = {
            default_target: fmt.parseNumber(document.getElementById('set_default_target')?.value) || (state.profile?.default_target || 10000),
            receipt_text: document.getElementById('set_receipt_text') ? document.getElementById('set_receipt_text').value.trim() : (state.profile?.receipt_text || ''),
        ...
        const invoiceSettings = {
            brand_color: document.getElementById('set_inv_brand_color')?.value || (state.profile?.invoice_settings?.brand_color || '#4f46e5'),
            logo_data: document.getElementById('set_inv_logo_data')?.value || (state.profile?.invoice_settings?.logo_data || null),
            tax_pin: document.getElementById('set_inv_tax_pin') ? document.getElementById('set_inv_tax_pin').value.trim() : (state.profile?.invoice_settings?.tax_pin || ''),
            mobile_money_provider: document.getElementById('set_inv_momo_provider')?.value || (state.profile?.invoice_settings?.mobile_money_provider || 'M-Pesa'),
            mobile_money_till: document.getElementById('set_inv_momo_till') ? document.getElementById('set_inv_momo_till').value.trim() : (state.profile?.invoice_settings?.mobile_money_till || ''),
            mobile_money_paybill: document.getElementById('set_inv_momo_paybill') ? document.getElementById('set_inv_momo_paybill').value.trim() : (state.profile?.invoice_settings?.mobile_money_paybill || ''),
            mobile_money_account_no: document.getElementById('set_inv_momo_account') ? document.getElementById('set_inv_momo_account').value.trim() : (state.profile?.invoice_settings?.mobile_money_account_no || ''),
            mobile_money_instructions: document.getElementById('set_inv_momo_instructions') ? document.getElementById('set_inv_momo_instructions').value.trim() : (state.profile?.invoice_settings?.mobile_money_instructions || ''),
            bank_name: document.getElementById('set_inv_bank_name') ? document.getElementById('set_inv_bank_name').value.trim() : (state.profile?.invoice_settings?.bank_name || ''),
            bank_account_no: document.getElementById('set_inv_bank_acc') ? document.getElementById('set_inv_bank_acc').value.trim() : (state.profile?.invoice_settings?.bank_account_no || ''),
            bank_swift: document.getElementById('set_inv_bank_swift') ? document.getElementById('set_inv_bank_swift').value.trim() : (state.profile?.invoice_settings?.bank_swift || ''),
            payment_terms: document.getElementById('set_inv_terms') ? document.getElementById('set_inv_terms').value.trim() : (state.profile?.invoice_settings?.payment_terms || 'Due upon receipt'),
            notes: document.getElementById('set_inv_notes') ? document.getElementById('set_inv_notes').value.trim() : (state.profile?.invoice_settings?.notes || 'Thank you for your business!')
        };

        const security = {
            two_factor: document.getElementById('set_two_factor')?.checked ?? (state.profile?.two_factor ?? false)
        };
        ...
        try {
            ...
            if (userEl) userEl.textContent = updatedProfile.full_name || state.currentUser;
...
window.togglePasswordChangeForm = function(forceState) {
    ...
    if (!container) return;
    const isOpening = (typeof forceState === 'boolean') ? forceState : container.classList.contains('hidden');
    if (isOpening) {
        container.innerHTML = `
            <div class="grid grid-cols-1 sm:grid-cols-2 gap-4">
                <div>
                    ...
                    <div class="relative">
                        <input type="password" id="sec_new_password" placeholder="Enter new password (min. 6 chars)" autocomplete="new-password" class="form-input w-full rounded-xl pr-10 text-sm">
                ...
                <div>
                    ...
                    <div class="relative">
                        <input type="password" id="sec_confirm_password" placeholder="Repeat new password" autocomplete="new-password" class="form-input w-full rounded-xl pr-10 text-sm">
