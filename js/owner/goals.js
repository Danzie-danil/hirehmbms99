...
import { dbBranches, dbGoals } from '../db.js';
...
export async function renderGoalsModule() {
    ...
    try {
        ...
        const [goals, metrics] = await Promise.all([
            dbGoals.fetchAll(ownerId),
            dbGoals.fetchMetrics(branchIds, currentMonth)
        ]);
...
export async function renderAddGoalView() {
    ...
    setTimeout(() => {
        if (window.hydrateFormDraft) window.hydrateFormDraft('ownerGoalDraft', area);
        if (window.attachFormDraftAutoSave) window.attachFormDraftAutoSave('ownerGoalDraft', area);
    }, 50);
}
...
window.submitGoal = async function () {
    ...
    try {
        ...
        if (window.clearFormDraft) window.clearFormDraft('ownerGoalDraft');
