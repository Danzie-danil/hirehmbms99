...
export async function renderStockTransfersModule() {
    ...
    try {
        const ownerId = state.ownerId || state.ownerIdForBranch;
        const [allBranches, transfers] = await Promise.all([
            window.dbBranches ? window.dbBranches.fetchAll(ownerId).catch(() => []) : [],
            window.dbStockTransfers ? window.dbStockTransfers.fetchAllForBranch(state.branchId).catch(() => []) : []
        ]);
...
window.openTransferModal = async function () {
    const ownerId = state.ownerId || state.ownerIdForBranch;
    const allBranches = await (window.dbBranches ? window.dbBranches.fetchAll(ownerId).catch(() => []) : []);
    ...
    const invRes = await (window.dbInventory ? window.dbInventory.fetchAll(state.branchId, { pageSize: 500 }).catch(() => ({ items: [] })) : { items: [] });
    const rawInventory = Array.isArray(invRes) ? invRes : (invRes.items || []);
    const inventory = rawInventory.filter(i => !i.is_isolated && i.isolation_status !== 'isolated');
...
window.submitStockTransfer = async function () {
    ...
    try {
        ...
        if (window.dbStockTransfers && typeof window.dbStockTransfers.create === 'function') {
            await window.dbStockTransfers.create(transferPayload);
        }
