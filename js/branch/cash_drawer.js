...
export async function renderCashDrawerModule() {
    ...
    try {
        ...
        const [drawerArr, [salesRes, cashSalesRes]] = await Promise.all([
            supabaseClient
                .from('cash_drawers')
                .select('*')
        ...
        if (drawer) {
            try {
                const { data: txData } = await Promise.race([
                    supabaseClient
        ...
        container.innerHTML = `
        <div class="space-y-4 sm:space-y-5 slide-in" id="cashDrawerShell">
            ...
            <div class="bg-white dark:bg-gray-800 rounded-2xl border border-gray-200 dark:border-gray-700 p-3.5 sm:p-5 flex flex-col sm:flex-row sm:items-center sm:justify-between gap-3">
                <div class="flex items-center gap-2.5 sm:gap-3 min-w-0 flex-1">
                    <div class="w-9 h-9 sm:w-11 sm:h-11 rounded-xl bg-emerald-50 dark:bg-emerald-950/60 border border-emerald-100 dark:border-emerald-900/50 flex items-center justify-center flex-shrink-0 text-emerald-600 dark:text-emerald-400">
                        <i data-lucide="banknote" class="w-4 h-4 sm:w-6 sm:h-6"></i>
                    </div>
                    <div class="min-w-0 flex-1">
                        <div class="flex items-center gap-2 flex-wrap">
                            <h2 class="text-sm sm:text-lg font-black text-gray-900 dark:text-white tracking-tight truncate">${window.t('till_reconciliation_title', 'Till Reconciliation')}</h2>
                            <span class="px-2 py-0.5 rounded-full text-[9px] sm:text-[10px] font-extrabold uppercase tracking-wider ${isOpen ? 'bg-emerald-50 dark:bg-emerald-900/40 text-emerald-600 dark:text-emerald-400 border border-emerald-200 dark:border-emerald-800' : isClosed ? 'bg-gray-100 dark:bg-gray-700 text-gray-600 dark:text-gray-300' : 'bg-amber-50 dark:bg-amber-900/40 text-amber-600 dark:text-amber-400 border border-amber-200 dark:border-amber-800'}">
                                ${isOpen ? window.t('drawer_status_open', 'Open') : isClosed ? window.t('drawer_status_closed', 'Closed') : window.t('till_not_open', 'Not Opened')}
                            </span>
                        </div>
                ...
                <div class="flex items-center gap-1.5 sm:gap-2 w-full sm:w-auto">
                    ${!drawer ? `
                    <button onclick="openDrawerSession()" class="flex-1 sm:flex-initial inline-flex items-center justify-center gap-1.5 px-3 sm:px-3.5 py-1.5 sm:py-2 bg-emerald-600 text-white rounded-xl text-[11px] sm:text-xs font-bold hover:bg-emerald-700 transition-all shadow-xs cursor-pointer whitespace-nowrap">
                        <i data-lucide="lock-open" class="w-3 h-3 sm:w-3.5 sm:h-3.5"></i>
                        <span>${window.t('reopen_drawer', 'Open Till')}</span>
                    </button>` : isOpen ? `
                    <div class="flex items-center gap-1.5 flex-1 sm:flex-initial">
                        <button onclick="openCashTxModal('in')" class="px-2.5 sm:px-3 py-1.5 sm:py-2 bg-gray-50 dark:bg-gray-700/60 border border-gray-200 dark:border-gray-700 rounded-xl text-[11px] sm:text-xs font-bold text-emerald-600 dark:text-emerald-400 hover:bg-emerald-50 dark:hover:bg-emerald-950/30 transition-colors flex items-center justify-center gap-1 cursor-pointer flex-1 sm:flex-initial">
                            <i data-lucide="plus-circle" class="w-3 h-3 sm:w-3.5 sm:h-3.5"></i>
                            <span>In</span>
                        </button>
                        <button onclick="openCashTxModal('out')" class="px-2.5 sm:px-3 py-1.5 sm:py-2 bg-gray-50 dark:bg-gray-700/60 border border-gray-200 dark:border-gray-700 rounded-xl text-[11px] sm:text-xs font-bold text-red-600 dark:text-red-400 hover:bg-red-50 dark:hover:bg-red-950/30 transition-colors flex items-center justify-center gap-1 cursor-pointer flex-1 sm:flex-initial">
                            <i data-lucide="minus-circle" class="w-3 h-3 sm:w-3.5 sm:h-3.5"></i>
                            <span>Out</span>
                        </button>
                    </div>
                    <button onclick="renderTillReconciliationView('${drawer.id}', ${expectedCash})" class="flex-1 sm:flex-initial inline-flex items-center justify-center gap-1.5 px-3 sm:px-3.5 py-1.5 sm:py-2 bg-gray-900 dark:bg-white dark:text-gray-900 text-white rounded-xl text-[11px] sm:text-xs font-bold hover:bg-gray-800 transition-all shadow-xs cursor-pointer whitespace-nowrap">
                        <i data-lucide="calculator" class="w-3 h-3 sm:w-3.5 sm:h-3.5"></i>
                        <span>${window.t('close_till', 'Reconcile')}</span>
                    </button>` : ``}
                </div>
            </div>
            ...
            <div class="grid grid-cols-2 sm:grid-cols-2 lg:grid-cols-4 gap-2.5 sm:gap-3.5">
                ...
                <div class="relative bg-white dark:bg-gray-800 p-3 sm:p-4 rounded-2xl border border-gray-200 dark:border-gray-700 stat-card flex flex-col justify-between h-full min-w-0">
                    ...
                    <div class="flex items-center justify-between mb-1.5 pr-10">
                        <span class="text-[11px] font-bold text-gray-500 dark:text-gray-400 uppercase tracking-tight truncate block">${window.t('opening_float', 'Opening Float')}</span>
                    </div>
                    <div class="min-w-0 mt-auto pr-9">
                        <p class="text-dynamic-lg font-black text-gray-900 dark:text-white truncate leading-tight" title="${fmt.currency(openingFloat)}">${fmt.number(openingFloat)}</p>
                        <p class="text-[10px] text-gray-400 font-semibold mt-0.5 truncate">Base float</p>
                    </div>
                    <svg class="absolute bottom-2 right-2 w-5 h-3 text-gray-400 opacity-75" viewBox="0 0 36 24" fill="currentColor">
                        ...
                        <rect x="23" y="12" width="4.5" height="12" rx="1.5"/>
                        <rect x="30" y="12" width="4.5" height="12" rx="1.5"/>
                    </svg>
                </div>
                ...
                <div class="relative bg-white dark:bg-gray-800 p-3 sm:p-4 rounded-2xl border border-gray-200 dark:border-gray-700 stat-card flex flex-col justify-between h-full min-w-0">
                    ...
                    <div class="flex items-center justify-between mb-1.5 pr-10">
                        <span class="text-[11px] font-bold text-gray-500 dark:text-gray-400 uppercase tracking-tight truncate block">${window.t('cash_sales_today', 'Cash Inflow')}</span>
                    </div>
                    <div class="min-w-0 mt-auto pr-9">
                        <p class="text-dynamic-lg font-black text-emerald-600 dark:text-emerald-400 truncate leading-tight" title="${fmt.currency(cashSales)}">${fmt.number(cashSales)}</p>
                        <p class="text-[10px] text-emerald-600 dark:text-emerald-400 font-semibold mt-0.5 truncate">Physical cash collected</p>
                    </div>
                ...
                <div class="relative bg-white dark:bg-gray-800 p-3 sm:p-4 rounded-2xl border border-gray-200 dark:border-gray-700 stat-card flex flex-col justify-between h-full min-w-0">
                    ...
                    <div class="flex items-center justify-between mb-1.5 pr-10">
                        <span class="text-[11px] font-bold text-indigo-600 dark:text-indigo-400 uppercase tracking-tight truncate block">${window.t('expected_cash', 'Expected in Till')}</span>
                    </div>
                    <div class="min-w-0 mt-auto pr-9">
                        <p class="text-dynamic-lg font-black text-indigo-600 dark:text-indigo-400 truncate leading-tight" title="${fmt.currency(expectedCash)}">${fmt.number(expectedCash)}</p>
                        <p class="text-[10px] text-gray-400 font-semibold mt-0.5 truncate">Float + Inflow - Payouts</p>
                    </div>
                ...
                <div class="relative bg-white dark:bg-gray-800 p-3 sm:p-4 rounded-2xl border border-gray-200 dark:border-gray-700 stat-card flex flex-col justify-between h-full min-w-0">
                    <div class="absolute -top-2.5 right-3.5 px-2 py-0.5 rounded-full text-[9px] font-black uppercase tracking-wider bg-slate-100 dark:bg-gray-700 text-slate-700 dark:text-slate-300 border border-slate-300 dark:border-gray-600 shadow-2xs z-10">${currencySymbol}</div>
                    <div class="flex items-center justify-between mb-1.5 pr-10">
                        <span class="text-[11px] font-bold text-gray-500 dark:text-gray-400 uppercase tracking-tight truncate block">${window.t('variance_label', 'Variance')}</span>
                    </div>
                    <div class="min-w-0 mt-auto pr-9">
                        <p class="text-dynamic-lg font-black ${variantClass || 'text-gray-900 dark:text-white'} truncate leading-tight" title="${variance !== null ? fmt.currency(variance) : '0'}">
                            ${variance !== null ? `${variance >= 0 ? '+' : ''}${fmt.number(variance)}` : 'Balanced'}
                        </p>
                        <p class="text-[10px] font-semibold mt-0.5 truncate ${variance === null ? 'text-gray-400' : variantClass}">
                            ${variance === null ? 'Till active' : variance === 0 ? 'Exact count match' : variance > 0 ? 'Over' : 'Short'}
                        </p>
                    </div>
                    <div class="absolute bottom-2 right-2 w-5.5 h-5.5 rounded-full ${variance === 0 ? 'bg-emerald-50 text-emerald-600' : variance > 0 ? 'bg-blue-50 text-blue-600' : 'bg-gray-100 text-gray-500'} flex items-center justify-center text-[10px] font-black shadow-2xs">
                        <i data-lucide="${variance === null || variance === 0 ? 'check' : 'alert-circle'}" class="w-3 h-3"></i>
                    </div>
                </div>
            </div>
            ...
            <div class="bg-gray-50/70 dark:bg-gray-900/50 border border-gray-200 dark:border-gray-700 rounded-2xl p-4 sm:p-5">
                <div class="flex items-center gap-3 mb-3.5">
                    <div class="w-9 h-9 bg-gray-200 dark:bg-gray-700 rounded-xl flex items-center justify-center text-gray-600 dark:text-gray-300">
                        <i data-lucide="lock" class="w-4 h-4"></i>
                    </div>
                    <div>
                        <h3 class="font-black text-gray-900 dark:text-white text-sm">${window.t('till_close_report', 'Till Reconciliation Summary')}</h3>
                        <p class="text-xs text-gray-500 dark:text-gray-400">${window.t('till_already_closed', "Today's till has been verified and balanced.")}</p>
                    </div>
                </div>
                <div class="grid grid-cols-2 sm:grid-cols-3 gap-2.5 sm:gap-3 text-sm">
                    <div class="bg-white dark:bg-gray-800 p-3 rounded-xl border border-gray-200 dark:border-gray-700">
                        ...
                        <p class="font-black text-gray-900 dark:text-white mt-0.5">${fmt.currency(openingFloat)}</p>
                    </div>
                    <div class="bg-white dark:bg-gray-800 p-3 rounded-xl border border-gray-200 dark:border-gray-700">
                        <p class="text-[10px] text-gray-400 font-bold uppercase">${window.t('expected_cash', 'Expected Cash')}</p>
                        <p class="font-black text-indigo-600 dark:text-indigo-400 mt-0.5">${fmt.currency(expectedCash)}</p>
                    </div>
                    <div class="bg-white dark:bg-gray-800 p-3 rounded-xl border border-gray-200 dark:border-gray-700 col-span-2 sm:col-span-1">
                        <p class="text-[10px] text-gray-400 font-bold uppercase">${window.t('closing_count', 'Physical Cash Counted')}</p>
                        <p class="font-black text-emerald-600 dark:text-emerald-400 mt-0.5">${fmt.currency(drawer.closing_cash)}</p>
                    </div>
                </div>
                ${drawer.discrepancy_note ? `
                <div class="mt-3 p-3 bg-amber-50 dark:bg-amber-950/40 border border-amber-200 dark:border-amber-800 rounded-xl">
                    <p class="text-[10px] font-bold uppercase text-amber-700 dark:text-amber-300 mb-0.5">${window.t('discrepancy_note', 'Reconciliation Note')}</p>
                    <p class="text-xs text-amber-900 dark:text-amber-200 italic">${drawer.discrepancy_note}</p>
                </div>` : ''}
            </div>` : ''}

            <!-- Cash Movements Bento Container -->
            <div class="bg-white dark:bg-gray-800 rounded-2xl border border-gray-200 dark:border-gray-700 p-4 sm:p-5">
                <div class="flex items-center justify-between mb-3.5">
                    <div>
                        <h3 class="text-xs sm:text-sm font-bold text-gray-900 dark:text-white uppercase tracking-wider">${window.t('payment_history', 'Cash Transactions & Flow')}</h3>
                        <p class="text-[11px] text-gray-400 font-medium">Logged cash-in and payout movement history</p>
                    </div>
                    <span class="text-xs text-gray-400 font-medium">${cashTx.length} records</span>
                </div>

                ${cashTx.length === 0 ? `
                <div class="py-8 text-center border-2 border-dashed border-gray-200 dark:border-gray-700 rounded-2xl">
                    <i data-lucide="inbox" class="w-8 h-8 text-gray-300 dark:text-gray-600 mx-auto mb-2"></i>
                    <p class="text-gray-400 text-xs font-medium">${drawer ? window.t('no_payments_yet', 'No cash movements recorded yet') : window.t('till_not_open', 'Open the till to start tracking')}</p>
                </div>` : `
                <div class="space-y-2">
                    ${cashTx.map(t => `
                    <div class="bg-gray-50/70 dark:bg-gray-900/50 hover:bg-white dark:hover:bg-gray-800 border border-gray-200 dark:border-gray-700 rounded-xl p-3 sm:p-4 flex items-center justify-between gap-3 transition-colors">
                        <div class="flex items-center gap-3 min-w-0">
                            <div class="w-8 h-8 rounded-lg flex items-center justify-center flex-shrink-0 ${t.type === 'in' ? 'bg-emerald-50 dark:bg-emerald-950/60 text-emerald-600 border border-emerald-200 dark:border-emerald-800' : 'bg-red-50 dark:bg-red-950/60 text-red-600 border border-red-200 dark:border-red-800'}">
                                <i data-lucide="${t.type === 'in' ? 'arrow-down-left' : 'arrow-up-right'}" class="w-4 h-4"></i>
                            </div>
                            <div class="min-w-0">
                                <p class="text-xs sm:text-sm font-bold text-gray-900 dark:text-white truncate">${t.reason || (t.type === 'in' ? 'Cash In' : 'Cash Out')}</p>
                                <p class="text-[10px] text-gray-400">${fmt.dateTime(t.created_at)}</p>
                            </div>
                        </div>
                        <span class="font-black text-xs sm:text-sm ${t.type === 'in' ? 'text-emerald-600 dark:text-emerald-400' : 'text-red-600 dark:text-red-400'} shrink-0">${t.type === 'in' ? '+' : '-'}${fmt.currency(t.amount)}</span>
                    </div>`).join('')}
                </div>`}
            </div>
        </div>`;
        ...
        console.error('[CashDrawer] Error loading cash drawer:', err);
        const area = document.getElementById('mainContent');
        if (area) {
            if (typeof window.renderModuleOfflineState === 'function') {
                area.innerHTML = window.renderModuleOfflineState({
                    viewId: 'cash_drawer',
                    title: 'Till Reconciliation',
                    entityName: 'Cash Till & Transactions',
                    retryAction: 'window.renderCashDrawerModule()'
                });
                if (window.lucide) window.lucide.createIcons();
            } else {
                area.innerHTML = `<div class="py-20 text-center text-red-500 font-bold">Failed to load cash drawer: ${err.message}</div>`;
            }
        }
    }
}
