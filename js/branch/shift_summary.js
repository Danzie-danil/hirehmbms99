...
export async function renderShiftSummaryModule() {
    ...
    try {
        ...
        const [salesRes, expRes, attRes, drawerRes] = await Promise.all([
            supabaseClient.from('sales').select('amount, payment, items, quantity').eq('branch_id', state.branchId).gte('created_at', today),
        ...
        const isServiceSale = (s) => s.item_type === 'service' || (s.items && typeof s.items === 'string' && (s.items.toLowerCase().includes('(service)') || s.items.toLowerCase().includes('service')));
        const productSales = sales.filter(s => !isServiceSale(s));
        const serviceSales = sales.filter(s => isServiceSale(s));

        const productRevenue = productSales.reduce((s, r) => s + Number(r.amount), 0);
        const serviceRevenue = serviceSales.reduce((s, r) => s + Number(r.amount), 0);
        const productUnits = productSales.reduce((s, r) => s + (Number(r.quantity) || 1), 0);
        ...
        <div class="space-y-4 slide-in">
            <div class="flex flex-wrap items-center justify-between gap-3">
                <div class="inline-flex items-center gap-2 bg-white dark:bg-gray-800 border border-gray-200 dark:border-gray-700 shadow-sm rounded-2xl p-1 pr-5">
                    <div class="bg-indigo-50 dark:bg-indigo-950/40 text-indigo-700 dark:text-indigo-300 px-4 py-2 rounded-xl text-xs font-bold uppercase tracking-wider">${window.t('shift_summary_title', 'Shift Summary')}</div>
                    <span class="text-xs text-gray-400 font-medium">${new Date().toLocaleDateString('en-US', { weekday: 'long', month: 'long', day: 'numeric' })}</span>
                </div>
                <button onclick="submitShiftSummary()" class="flex items-center gap-2 px-5 py-2.5 bg-[#475B6E] hover:bg-[#394958] text-white rounded-xl text-sm font-bold shadow-lg transition-all cursor-pointer">
            ...
            <!-- Revenue Source Breakdown: Products vs Services -->
            <div class="grid grid-cols-1 sm:grid-cols-2 gap-3 sm:gap-4">
                <div class="bg-white dark:bg-gray-800 p-4 sm:p-5 rounded-2xl border border-gray-100 dark:border-gray-700 shadow-sm flex items-center justify-between gap-3">
                    <div class="flex items-center gap-3 min-w-0">
                        ...
                        <div class="min-w-0">
                            <p class="text-xs font-bold text-gray-500 dark:text-gray-400 uppercase tracking-wider">Physical Product Sales</p>
                            <p class="text-lg sm:text-xl font-black text-gray-900 dark:text-white truncate">${fmt.currency(productRevenue)}</p>
                        </div>
                    </div>
                    <div class="text-right shrink-0">
                        <span class="inline-flex items-center px-2 py-0.5 rounded-lg text-xs font-black bg-emerald-50 dark:bg-emerald-950/40 text-emerald-700 dark:text-emerald-300 border border-emerald-200/60 dark:border-emerald-800/60">${productUnits} units</span>
                    </div>
                </div>

                <div class="bg-white dark:bg-gray-800 p-4 sm:p-5 rounded-2xl border border-gray-100 dark:border-gray-700 shadow-sm flex items-center justify-between gap-3">
                    <div class="flex items-center gap-3 min-w-0">
                        <div class="w-10 h-10 rounded-xl bg-purple-50 dark:bg-purple-950/40 text-purple-600 dark:text-purple-400 flex items-center justify-center shrink-0">
                            <i data-lucide="wrench" class="w-5 h-5"></i>
                        </div>
                        <div class="min-w-0">
                            <p class="text-xs font-bold text-gray-500 dark:text-gray-400 uppercase tracking-wider">Service Offerings Revenue</p>
                            <p class="text-lg sm:text-xl font-black text-gray-900 dark:text-white truncate">${fmt.currency(serviceRevenue)}</p>
                        </div>
                    </div>
                    <div class="text-right shrink-0">
                        <span class="inline-flex items-center px-2 py-0.5 rounded-lg text-xs font-black bg-purple-50 dark:bg-purple-950/40 text-purple-700 dark:text-purple-300 border border-purple-200/60 dark:border-purple-800/60">${serviceCount} services</span>
                    </div>
                </div>
            </div>
            ...
            <div class="grid grid-cols-1 lg:grid-cols-2 gap-4">
                ...
                <div class="bg-white dark:bg-gray-800 rounded-2xl shadow-sm border border-gray-100 dark:border-gray-700 p-5">
                    <h3 class="font-bold text-gray-900 dark:text-white mb-4">${window.t('payment_breakdown_title', 'Payment Breakdown')}</h3>
                    <div class="space-y-3">
                        ${[
...
window.submitShiftSummary = async function () {
    ...
    try {
        ...
        const [sRes, eRes] = await Promise.all([
            supabaseClient.from('sales').select('amount, items, quantity').eq('branch_id', state.branchId).gte('created_at', new Date().toISOString().slice(0, 10)),
        ...
        const isServiceSale = (s) => s.item_type === 'service' || (s.items && typeof s.items === 'string' && (s.items.toLowerCase().includes('(service)') || s.items.toLowerCase().includes('service')));
        const prodSales = allSales.filter(s => !isServiceSale(s));
        const servSales = allSales.filter(s => isServiceSale(s));

        const totalSales = allSales.reduce((s, r) => s + Number(r.amount), 0);
        const prodTotal = prodSales.reduce((s, r) => s + Number(r.amount), 0);
        const servTotal = servSales.reduce((s, r) => s + Number(r.amount), 0);
        const totalExpenses = (eRes.data || []).reduce((s, r) => s + Number(r.amount), 0);

        const message = `Shift Summary — ${today}\n\nBranch: ${branch?.name || 'Branch'}\nTotal Sales: ${fmt.currency(totalSales)}\n • Physical Goods Sales: ${fmt.currency(prodTotal)} (${prodSales.length} txns)\n • Service Offerings: ${fmt.currency(servTotal)} (${servSales.length} services)\nTotal Expenses: ${fmt.currency(totalExpenses)}\nNet Cash: ${fmt.currency(totalSales - totalExpenses)}\n\nNotes: ${notes || '(none)'}`;
