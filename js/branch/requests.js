...
export async function renderBranchRequestsList() {
    ...
    try {
        ...
        listContainer.innerHTML = requests.map(req => {
            ...
            const isServiceReq = req.metadata?.item_type === 'service' || (req.subject && req.subject.toLowerCase().includes('service'));
            return `
                ...
                <div class="flex items-start justify-between gap-4 mb-4">
                    <div class="flex items-center gap-3">
                        <div class="w-10 h-10 ${badgeColors[req.status]} rounded-xl flex items-center justify-center flex-shrink-0">
                            <i data-lucide="${isServiceReq ? 'wrench' : (req.type.includes('inventory') ? 'package' : 'message-square')}" class="w-5 h-5"></i>
                        </div>
                        <div>
