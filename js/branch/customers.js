...
export function renderCustomersModule() {
        ...
        <div class="bg-white dark:bg-gray-800 rounded-2xl border border-gray-200 dark:border-gray-700 p-3.5 sm:p-5 flex flex-col sm:flex-row sm:items-center sm:justify-between gap-3">
            <div class="flex items-center gap-2.5 sm:gap-3 min-w-0 flex-1">
                ...
                <div class="min-w-0 flex-1">
                    <h2 class="text-sm sm:text-lg font-black text-gray-900 dark:text-white tracking-tight truncate">${window.t('customer_directory', 'Customer Directory')}</h2>
            ...
            <div class="flex items-center gap-1.5 sm:gap-2 w-full sm:w-auto">
                <button onclick="openModal('importCustomersInfo')" data-tooltip="Import customers from CSV" data-tooltip-title="Import CSV" class="flex-1 sm:flex-initial px-2.5 sm:px-3 py-1.5 sm:py-2 bg-gray-50 dark:bg-gray-700/60 border border-gray-200 dark:border-gray-700 rounded-xl text-[11px] sm:text-xs font-bold text-gray-700 dark:text-gray-300 hover:bg-indigo-50 dark:hover:bg-indigo-950/30 hover:text-indigo-600 transition-colors flex items-center justify-center gap-1.5 cursor-pointer whitespace-nowrap">
                    <i data-lucide="upload" class="w-3 h-3 sm:w-3.5 sm:h-3.5 text-gray-500"></i>
                    <span>${window.t('import_csv', 'Import CSV')}</span>
                </button>
                <button onclick="openModal('addCustomer')" data-tooltip="Register customer profile" data-tooltip-title="New Customer" data-tooltip-variant="indigo" class="flex-1 sm:flex-initial inline-flex items-center justify-center gap-1.5 px-3 sm:px-3.5 py-1.5 sm:py-2 rounded-xl text-[11px] sm:text-xs font-bold text-white bg-indigo-600 hover:bg-indigo-700 transition-all cursor-pointer shadow-xs whitespace-nowrap">
                    <i data-lucide="user-plus" class="w-3 h-3 sm:w-3.5 sm:h-3.5"></i>
                    <span>${window.t('add_customer', 'Add Customer')}</span>
                </button>
            </div>
        </div>
    ...
    Promise.all([
        dbCustomers.fetchAll(branchId, {
            page: customersPageState.page,
            pageSize: customersPageState.pageSize
        }),
        dbCustomerTags.fetchAll(branchId).catch(() => [])
    ]).then(([res, tags]) => {
        ...
        const totalDebt = customers.reduce((s, c) => s + Number(c.credit_balance || 0), 0);
        const debtorsCount = customers.filter(c => Number(c.credit_balance || 0) > 0).length;
            ...
            <div class="grid grid-cols-2 sm:grid-cols-2 lg:grid-cols-3 gap-2.5 sm:gap-3.5">
                ...
                <div class="relative bg-white dark:bg-gray-800 p-3 sm:p-4 rounded-2xl border border-gray-200 dark:border-gray-700 stat-card flex flex-col justify-between h-full min-w-0">
                    <div class="flex items-center justify-between mb-1.5 pr-10">
                        <span class="text-[11px] font-bold text-gray-500 dark:text-gray-400 uppercase tracking-tight truncate block">${window.t('total_customers', 'Directory Total')}</span>
                    </div>
                    <div class="min-w-0 mt-auto pr-9">
                        <p class="text-dynamic-lg font-black text-gray-900 dark:text-white truncate leading-tight">${customersPageState.totalCount}</p>
                        <p class="text-[10px] text-indigo-600 dark:text-indigo-400 font-semibold mt-0.5 truncate">Profiles</p>
                    </div>
                ...
                <div class="relative bg-white dark:bg-gray-800 p-3 sm:p-4 rounded-2xl border border-gray-200 dark:border-gray-700 stat-card flex flex-col justify-between h-full min-w-0">
                    ...
                    <div class="flex items-center justify-between mb-1.5 pr-10">
                        <span class="text-[11px] font-bold text-red-600 dark:text-red-400 uppercase tracking-tight truncate block">${window.t('customer_debt', 'Credit Receivables')}</span>
                    </div>
                    <div class="min-w-0 mt-auto pr-9">
                        <p class="text-dynamic-lg font-black text-red-600 dark:text-red-400 truncate leading-tight" title="${fmt.currency(totalDebt)}">${fmt.number(totalDebt)}</p>
                        <p class="text-[10px] text-gray-400 font-semibold mt-0.5 truncate">${debtorsCount} active balances</p>
                    </div>
                    <svg class="absolute bottom-2 right-2 w-5.5 h-3.5 opacity-75" viewBox="0 0 40 24" fill="none">
                        <path d="M2 18 L10 12 L18 16 L26 8 L38 4" stroke="#EF4444" stroke-width="2.5" stroke-linecap="round" stroke-linejoin="round"/>
                        <circle cx="38" cy="4" r="2.5" fill="#EF4444"/>
                    </svg>
                </div>

                <!-- Customers on Current Page -->
                <div class="relative bg-white dark:bg-gray-800 p-3 sm:p-4 rounded-2xl border border-gray-200 dark:border-gray-700 stat-card flex flex-col justify-between h-full col-span-2 sm:col-span-1 min-w-0">
                    <div class="flex items-center justify-between mb-1.5 pr-10">
                        <span class="text-[11px] font-bold text-gray-500 dark:text-gray-400 uppercase tracking-tight truncate block">${window.t('items_on_page', 'Displayed')}</span>
                    </div>
                    <div class="min-w-0 mt-auto pr-9">
                        <p class="text-dynamic-lg font-black text-gray-900 dark:text-white truncate leading-tight">${customers.length}</p>
                        <p class="text-[10px] text-emerald-600 dark:text-emerald-400 font-semibold mt-0.5 truncate">Page records</p>
                    </div>
                    <div class="absolute bottom-2 right-2 w-5.5 h-5.5 rounded-full bg-emerald-50 dark:bg-emerald-950/60 border border-emerald-200 dark:border-emerald-800 flex items-center justify-center text-emerald-600 text-[10px] font-black shadow-2xs">
                        <i data-lucide="list" class="w-3 h-3"></i>
                    </div>
                </div>
            </div>

            <!-- Customer Directory Bento Container -->
            <div class="bg-white dark:bg-gray-800 rounded-2xl border border-gray-200 dark:border-gray-700 p-4 sm:p-5">
                <div class="flex items-center justify-between mb-3.5">
                    <div>
                        <h3 class="text-xs sm:text-sm font-bold text-gray-900 dark:text-white uppercase tracking-wider">${window.t('customer_directory', 'Customer List')}</h3>
                        <p class="text-[11px] text-gray-400 font-medium">Directory profiles and credit balances</p>
                    </div>
                ...
                <div class="relative mb-3">
                    ...
                    <input type="text" placeholder="${window.t('search_customers', 'Search customers by name, phone, or tags...')}" oninput="filterList('customersList', this.value)" class="w-full pl-10 pr-3.5 py-2 bg-gray-50 dark:bg-gray-900 border border-gray-200 dark:border-gray-700 rounded-xl text-xs sm:text-sm focus:bg-white focus:ring-2 focus:ring-indigo-500/20 focus:border-indigo-500 outline-none transition-all placeholder-gray-400" style="padding-left: 2.75rem !important;">
                </div>
                ...
                <div class="flex flex-wrap items-center justify-between bg-gray-50/70 dark:bg-gray-900/50 border border-gray-100 dark:border-gray-800 rounded-xl p-2.5 mb-3.5 gap-2">
                    <div class="flex items-center gap-2.5 pl-1">
                        ...
                        <span class="text-xs font-bold text-gray-800 dark:text-gray-200">${window.t('select_all', 'Select All')} <span id="customersSelectedCount" class="font-normal text-xs text-gray-400 ml-1 hidden sm:inline-block">0 selected</span></span>
                    </div>
                    <div class="flex flex-wrap items-center gap-1.5">
                        <button id="btnBulkDeleteCustomers" disabled onclick="bulkDeleteSelectedCustomers()" data-tooltip="Delete selected customer profiles" data-tooltip-title="Bulk Delete" data-tooltip-variant="rose" class="flex items-center gap-1.5 px-2.5 py-1 bg-white dark:bg-gray-800 border border-gray-200 dark:border-gray-700 rounded-lg text-xs font-semibold text-gray-700 dark:text-gray-300 hover:bg-red-50 hover:text-red-600 transition-colors disabled:opacity-50 disabled:cursor-not-allowed cursor-pointer">
                ...
                <div class="space-y-2.5" id="customersList">
                    ...
                    ` : customers.map(c => {
                        ...
                        const initials = c.name.split(' ').map(n => n[0]).join('').substring(0, 2).toUpperCase() || 'CU';
                        return `
                        <div onclick="openDetailsModal('customer', '${c.id}')" data-search="${c.name.toLowerCase()} ${(c.phone || '').toLowerCase()} ${(c.email || '').toLowerCase()}" class="bg-gray-50/70 dark:bg-gray-900/50 hover:bg-white dark:hover:bg-gray-800 border border-gray-200 dark:border-gray-700 rounded-xl p-3 sm:p-4 flex items-center justify-between gap-3 transition-colors group relative cursor-pointer">
                            <div class="flex items-center gap-3 min-w-0" onclick="event.stopPropagation()">
                                ...
                                <div class="w-8 h-8 rounded-lg ${hasDebt ? 'bg-red-50 dark:bg-red-950/60 text-red-600 border border-red-200 dark:border-red-800' : 'bg-indigo-50 dark:bg-indigo-900/40 text-indigo-600 dark:text-indigo-400 border border-indigo-200 dark:border-indigo-800'} font-bold text-[11px] flex items-center justify-center flex-shrink-0">
                                    ${initials}
                                </div>
                                <div class="min-w-0">
                                    <div class="flex items-center gap-1.5 flex-wrap">
                                        <h4 class="font-bold text-gray-900 dark:text-white text-xs sm:text-sm truncate">${c.name}</h4>
                                        ${hasDebt ? `<span class="bg-red-100 dark:bg-red-900/40 text-red-600 dark:text-red-300 text-[9px] px-1.5 py-0.5 rounded font-black uppercase tracking-wider shrink-0">${window.t('customer_debt', 'Debt')}: ${fmt.currency(c.credit_balance)}</span>` : ''}
                                        <span class="bg-amber-50 dark:bg-amber-950/40 text-amber-700 dark:text-amber-300 text-[9px] px-1.5 py-0.5 rounded font-bold shrink-0">${c.loyalty_points || 0} pts</span>
                                    </div>
                                    <p class="text-[10px] text-gray-400 flex items-center gap-1.5 mt-0.5">
                                        <span>${c.phone || window.t('phone', 'No phone')}</span>
                                        ${tags.filter(t => t.customer_id === c.id).map(t => `<span class="text-indigo-600 dark:text-indigo-400 font-semibold">#${t.tag}</span>`).join(' ')}
                                    </p>
                                </div>
                            </div>
                            ...
                            <div class="flex items-center gap-2 shrink-0" onclick="event.stopPropagation()">
                                ...
                                <div class="flex items-center gap-1">
                                    ...
                                    <button onclick="shareWhatsAppReceiptByCustomerId('${c.id}')"
                                        class="p-1.5 bg-green-50 dark:bg-green-900/20 border border-green-200 dark:border-green-800 text-green-700 dark:text-green-400 rounded-lg text-xs font-bold transition-all hover:bg-green-100 cursor-pointer">
                                        <i data-lucide="message-circle" class="w-3.5 h-3.5"></i>
                                    </button>` : ''}
                                    <button onclick="openPartialPaymentView('${c.id}')"
                                        class="px-2.5 py-1 bg-indigo-600 hover:bg-indigo-700 text-white rounded-lg text-xs font-bold transition-all shadow-xs cursor-pointer">
                                        Pay
                                    </button>
                                </div>` : `
                                <span class="text-[10px] text-gray-400">${fmt.dateTime(c.created_at)}</span>
                                `}
                            </div>
                        </div>`;
                    }).join('')}
...
window.submitPartialPayment = async function () {
    ...
    try {
        if (window.dbCustomers && typeof window.dbCustomers.updateCredit === 'function') {
            await window.dbCustomers.updateCredit(customerId, newBalance, {
                branch_id: state.branchId,
