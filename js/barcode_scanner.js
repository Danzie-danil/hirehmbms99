...
import { dbInventory, dbCentralInventory } from './db.js';
...
async function handleDetectedBarcode(code, onDetectedCallback) {
    if (navigator.vibrate) navigator.vibrate(120);
    if (window.playSound) playSound('pop-alert');

    // Fast product lookup for visual match HUD
    let matchedItem = null;
    try {
        const rawSku = String(code).trim().toLowerCase();
        if (state.branchId) {
            const res = await dbInventory.fetchAll(state.branchId, { pageSize: 10000 });
            matchedItem = (res.items || []).find(i => (i.sku || '').trim().toLowerCase() === rawSku || (i.name || '').trim().toLowerCase() === rawSku);
        }
        if (!matchedItem && state.role === 'owner' && state.ownerId) {
            const centralItems = await dbCentralInventory.fetchAll(state.ownerId);
            matchedItem = (centralItems || []).find(i => (i.sku || '').trim().toLowerCase() === rawSku || (i.name || '').trim().toLowerCase() === rawSku);
        }
    } catch (e) {}

    const statusEl = document.getElementById('barcodeScannerStatus');
    if (statusEl && matchedItem) {
        const priceFormatted = window.fmt ? window.fmt.currency(matchedItem.retail_price || matchedItem.price || 0) : '';
        const stockQty = matchedItem.quantity !== undefined ? `${matchedItem.quantity} in stock` : '';
        statusEl.className = 'absolute bottom-3 inset-x-3 bg-emerald-950/90 border border-emerald-500/80 backdrop-blur-md rounded-xl p-3 text-center text-white text-xs font-bold z-20 shadow-xl';
        statusEl.innerHTML = `
            <div class="flex items-center justify-center gap-2 text-emerald-400 font-black text-sm">
                <i data-lucide="check-circle" class="w-4 h-4"></i>
                <span class="truncate">${matchedItem.name}</span>
            </div>
            <div class="text-[11px] text-gray-300 font-medium mt-0.5">
                SKU: ${code} ${priceFormatted ? `· ${priceFormatted}` : ''} ${stockQty ? `· ${stockQty}` : ''}
            </div>`;
        if (window.lucide) lucide.createIcons({ scope: statusEl });
    ...
    if (onDetectedCallback && typeof window[onDetectedCallback] === 'function') {
        window[onDetectedCallback](code, matchedItem ? matchedItem.name : null);
    } else if (typeof window.handleBarcodeScan === 'function') {
        window.handleBarcodeScan(code);
    } else {
        const title = matchedItem ? `${matchedItem.name} (${code})` : code;
        if (typeof showToast === 'function') {
            showToast(`${window.t ? window.t('barcode_detected', 'Barcode detected') : 'Barcode detected'}: ${title}`, 'success');
        }
    }
}
